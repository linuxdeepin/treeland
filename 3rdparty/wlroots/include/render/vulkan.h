...
struct wlr_vk_renderer {
	...
	// struct wlr_vk_texture * parallel to the barrier arrays above. A deferred
	// barrier that is discarded without being recorded (batch abort, or a
	// flush without a command buffer) must roll the per-texture ownership
	// bookkeeping back, otherwise qt_sampling_acquired would claim a transfer
	// that never happened.
	struct wl_array texture_acquire_textures; // struct wlr_vk_texture *
	struct wl_array texture_release_textures; // struct wlr_vk_texture *
	...
	// with dedup, cleared by waylib_vk_renderer_reset_frame_alias_lists() at
	// the compositor frame boundary: same-queue submission order already
	// covers next-frame dependencies, so the lists only need to span one
	// frame's recording. Stale entries would only ever add a conservative
	// extra dependency, never remove one.
	struct wl_array frame_render_buffers;
	// wlr_buffer * sampled as a texture by the (Qt/QRhi) frame's command
	// buffer. Same lifecycle and conservative policy as
...
struct wlr_vk_texture {
	...
	bool qt_sampling_acquired; // waylib/Qt-only bookkeeping: the Qt Quick RHI
	// path (waylib_vk_renderer_prepare_texture_for_sampling() /
