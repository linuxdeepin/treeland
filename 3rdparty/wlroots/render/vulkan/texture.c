...
void vulkan_texture_destroy(struct wlr_vk_texture *texture) {
	...
	// Defensive unlink from the foreign-texture list: wlroots' render pass
	// submits that fail before its drain loop strand entries there, and
	// destroying the texture's buffer through that path would leave a dangling
	// node that the next successful submit's drain would touch (use-after-free).
	// pass.c inserts only when both link fields are NULL; self-linking marks
	// "not inserted" consistently for the wl_list_empty() checks elsewhere.
	if (texture->foreign_link.next != NULL &&
			!wl_list_empty(&texture->foreign_link)) {
		wl_list_remove(&texture->foreign_link);
	}
	texture->foreign_link.prev = &texture->foreign_link;
	texture->foreign_link.next = &texture->foreign_link;
...
void wlr_vk_texture_get_image_attribs(struct wlr_texture *texture,
	...
	// qt_sampling_acquired is the Qt-path owner of that ping-pong; 'owned'
	// is kept for the wlroots render-pass bookkeeping only (see header).
	attribs->layout = vk_texture->dmabuf_imported
		? ((vk_texture->owned || vk_texture->qt_sampling_acquired)
			? VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
...
static bool acquire_texture_sync_sem(struct wlr_vk_renderer *renderer,
	...
	for (size_t i = 0; i < count; i++) {
		if (sems[i].release_point <= cur_point) {
			if (sems[i].semaphore == VK_NULL_HANDLE) {
				// Reclaimed slot from a failed bridge submit (see
				// discard_texture_sync_sems): rebuild it here.
				VkSemaphoreCreateInfo semaphore_info = {
					.sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO,
				};
				VkResult cres = vkCreateSemaphore(renderer->dev->dev,
					&semaphore_info, NULL, &sems[i].semaphore);
				if (cres != VK_SUCCESS) {
					wlr_vk_error("vkCreateSemaphore", cres);
					return false;
				}
			}
...
// A failed bridge submit can leave the claimed semaphores holding a consumed
// temporary sync_fd import (or a submitted-wait whose execution never started,
// depending on where the submit failed). Any of those states makes the binary
// semaphore unusable for a later re-import, while its slot would otherwise be
// stuck at release_point=UINT64_MAX forever. Destroy and NULL the semaphores;
// acquire_texture_sync_sem() rebuilds reclaimed slots. All these semaphores
// have no pending wait (the submit that would wait on them failed before
// executing), so destruction is valid.
static void discard_texture_sync_sems(struct wlr_vk_renderer *renderer,
		struct wlr_vk_texture_sync_wait *waits, size_t count) {
	for (size_t i = 0; i < count; i++) {
		if (waits[i].semaphore_index == SIZE_MAX) {
			continue;
		}
		struct wlr_vk_texture_sync_sem *sem =
			texture_sync_sem_at(renderer, waits[i].semaphore_index);
		if (sem->semaphore != VK_NULL_HANDLE) {
			vkDestroySemaphore(renderer->dev->dev, sem->semaphore, NULL);
...
bool waylib_vk_renderer_flush_texture_sync_batch(struct wlr_renderer *wlr_renderer) {
	...
	if (imported_count < wait_count) {
		...
		bool gpu_ok = submit_texture_sync_bridge(renderer, waits, wait_infos,
			imported_count);
		if (!gpu_ok) {
			discard_texture_sync_sems(renderer, waits, imported_count);
		}
	...
	if (!ok) {
		discard_texture_sync_sems(renderer, waits, wait_count);
	}
	waylib_vk_renderer_abort_texture_sync_batch(wlr_renderer);
	return ok;
}
...
// Returns true when the texture's DMA-BUF was (or may have been) rendered as
// a color attachment by one of this renderer's recent command buffers. The
// producer accesses went through the render buffer's separate logical VkImage,
// which only aliases this texture's image in physical memory.
static bool is_frame_render_buffer(struct wlr_vk_renderer *renderer,
		struct wlr_buffer *buffer) {
	return vulkan_alias_list_contains(&renderer->frame_render_buffers, buffer);
}

// Extends the color-attachment writes of an aliasing render-buffer producer
// to the sampling about to be recorded for this texture. Neither the foreign
// ownership acquire (its source access mask is ignored) nor an image barrier
// naming the sampled texture can cover accesses made through the producer's
// different logical image: §Memory Aliasing requires a memory dependency and
// only a global (resource-free) barrier spans both aliases. The mask pair is
// the same shape the texture-sync bridge uses for staged uploads.
static void record_frame_render_barrier(struct wlr_vk_renderer *renderer,
	...
	if (renderer->texture_barrier_batch_active
		...
		renderer->frame_producer_alias_dep = true;
		deferred = true;
	}
	if (!deferred) {
		VkMemoryBarrier memory_barrier = {
	...
	if (!logged_frame_producer_barrier) {
		...
		wlr_log(WLR_DEBUG, "Vulkan texture sampling: recording a global "
			"alias dependency for a texture whose DMA-BUF was also written "
			"as a color attachment through another logical image");
	}
}
...
bool waylib_vk_renderer_prepare_texture_for_sampling(struct wlr_renderer *wlr_renderer,
	...
	if (texture->dmabuf_imported && !texture->qt_sampling_acquired) {
		if (texture->readback_acquired) {
			// A readback acquire holds the foreign->own transition of this
			// image. Qt must not sample it until that readback released it;
			// by construction readbacks run outside Qt frames, so failing
			// closed here only guards against future caller bugs.
			wlr_log(WLR_ERROR, "Vulkan texture readback acquire still in "
				"flight; refusing to sample it");
			return false;
		}
		...
		// Record the sampling side of the alias pair before touching any
		// barrier state: a later render-buffer acquire for the same
		// wlr_buffer writes through another logical image and needs a WAR
		// dependency against this sampling. Failing to track is handled
		// fail-closed so the WAR barrier can never be silently lost.
		if (texture->buffer != NULL
				&& !vulkan_alias_list_add(&renderer->frame_sampled_buffers,
					texture->buffer)) {
			wlr_log_errno(WLR_ERROR,
				"Failed to track Vulkan sampled buffer for aliasing");
			return false;
		}
		...
		if (renderer->texture_barrier_batch_active
			...
			struct wlr_vk_texture **track = slot != NULL ? wl_array_add(
				&renderer->texture_acquire_textures, sizeof(*track)) : NULL;
			if (slot != NULL && track != NULL) {
				*slot = barrier;
				*track = texture;
				deferred = true;
			} else {
				// Never leave a deferred barrier without its texture
				// bookkeeping: undo the partial adds and record it now.
				if (track != NULL) {
					renderer->texture_acquire_textures.size -= sizeof(*track);
				}
				if (slot != NULL) {
					renderer->texture_acquire_barriers.size -= sizeof(barrier);
				}
				wlr_log_errno(WLR_ERROR,
		...
		if (!deferred) {
			// The acquire's second scope must cover every stage the consumer
			// may sample from, not only the fragment stage.
		...
		texture->qt_sampling_acquired = true;
	}
...
bool waylib_vk_renderer_finish_texture_sampling(struct wlr_renderer *wlr_renderer,
	...
	if (renderer->texture_barrier_batch_active
		...
		struct wlr_vk_texture **track = slot != NULL ? wl_array_add(
			&renderer->texture_release_textures, sizeof(*track)) : NULL;
		if (slot != NULL && track != NULL) {
			*slot = barrier;
			*track = texture;
			deferred = true;
			...
			// bookkeeping: undo the partial adds and record it now.
			if (track != NULL) {
				renderer->texture_release_textures.size -= sizeof(*track);
			}
			if (slot != NULL) {
				renderer->texture_release_barriers.size -= sizeof(barrier);
			}
			wlr_log_errno(WLR_ERROR,
	...
	if (!deferred) {
		// The release's first scope must cover every stage that may still be
		// reading the texture, not only the fragment stage.
		vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
...
// texture's ownership bookkeeping claiming a transfer that did not happen. A
// dropped acquire leaves the image FOREIGN-owned (it must be re-acquired on
// the next use), while a dropped release leaves the graphics queue owning it
// (it must not be acquired again). Roll the per-texture flag back to match the
// image's real ownership before discarding the batch.
static void rollback_deferred_texture_barriers(struct wlr_vk_renderer *renderer) {
	const size_t acquire_count = renderer->texture_acquire_textures.size /
		sizeof(struct wlr_vk_texture *);
	const size_t release_count = renderer->texture_release_textures.size /
		sizeof(struct wlr_vk_texture *);

	struct wlr_vk_texture **texture;
	wl_array_for_each(texture, &renderer->texture_acquire_textures) {
		(*texture)->qt_sampling_acquired = false;
	}
	wl_array_for_each(texture, &renderer->texture_release_textures) {
		(*texture)->qt_sampling_acquired = true;
	}

	renderer->texture_acquire_textures.size = 0;
	renderer->texture_release_textures.size = 0;

	if (acquire_count > 0 || release_count > 0) {
		wlr_log(WLR_DEBUG, "Vulkan texture barrier batch rolled back: "
			"%zu dropped acquire(s), %zu dropped release(s)",
...
bool waylib_vk_renderer_flush_texture_barrier_batch(struct wlr_renderer *wlr_renderer,
	...
	// Test-only fault injection (one-shot): WAYLIB_VK_TEST_FLUSH_FAIL_AT=<n>
	// makes the n-th barrier batch flush of this process behave as if no
	// command buffer was available, so the rollback paths can be exercised on
	// a live compositor. Unset in normal operation; drop before upstreaming.
	{
		static long inject_fail_at = -1;
		if (inject_fail_at < 0) {
			const char *env = getenv("WAYLIB_VK_TEST_FLUSH_FAIL_AT");
			inject_fail_at = env != NULL ? strtol(env, NULL, 10) : 0;
			if (inject_fail_at < 0) {
				inject_fail_at = 0;
			}
		}
		if (inject_fail_at > 0) {
			...
			if (++flush_count == inject_fail_at) {
				wlr_log(WLR_INFO, "WAYLIB_VK_TEST_FLUSH_FAIL_AT: injecting a "
					"one-shot Vulkan texture barrier batch flush failure");
				cb = VK_NULL_HANDLE;
			}
		}
	}
	...
	if (cb == VK_NULL_HANDLE && (count > 0 || renderer->frame_producer_alias_dep)) {
		// Without a command buffer nothing can be recorded; discarding the
		// batch silently would desynchronize qt_sampling_acquired from the
		// image's real ownership. Roll the batch back and report the failure.
		wlr_log(WLR_ERROR, "Cannot record the pending Vulkan texture %s "
			"barrier batch without a command buffer; rolling it back",
			release ? "release" : "acquire");
		rollback_deferred_texture_barriers(renderer);
		renderer->texture_acquire_barriers.size = 0;
		renderer->texture_release_barriers.size = 0;
		renderer->frame_producer_alias_dep = false;
	...
	// Same-frame alias producer dependencies are only queued during the
	// acquire (pre-draw) phase and must be recorded after the
	// ownership-transfer acquire barriers above. A global memory barrier is
	// the only form whose access scope spans the producer's separate logical
	// image and this sampling (§Memory Aliasing).
	if (cb != VK_NULL_HANDLE && !release && renderer->frame_producer_alias_dep) {
		VkMemoryBarrier memory_barrier = {
			.sType = VK_STRUCTURE_TYPE_MEMORY_BARRIER,
			.srcAccessMask = VK_ACCESS_MEMORY_WRITE_BIT,
			.dstAccessMask = VK_ACCESS_MEMORY_READ_BIT,
...
// Submit a FOREIGN<->own queue-family ownership barrier (layout untouched,
// GENERAL, as every DMA-BUF import path in this renderer keeps it) on its own
// command buffer. acquire=true transfers ownership from the foreign producer
// to the graphics queue, permitting transfer-read and shader-read of the
// imported memory (which starts in an undefined layout from Vulkan's view);
// acquire=false releases it back. The later read_pixels() staging command
// buffer is submitted afterwards on the same queue: same-queue submission
// order provides the execution dependency between these two batches.
static bool submit_readback_barrier(struct wlr_vk_renderer *renderer,
		VkImage image, bool acquire) {
	VkResult res;
	struct wlr_vk_command_buffer *cb = vulkan_acquire_command_buffer(renderer);
	if (cb == NULL) {
		return false;
	...
	res = vkBeginCommandBuffer(cb->vk, &begin_info);
	if (res != VK_SUCCESS) {
		wlr_vk_error("vkBeginCommandBuffer", res);
		vulkan_reset_command_buffer(cb);
		return false;
	...
	const VkAccessFlags read_accesses = VK_ACCESS_TRANSFER_READ_BIT
	...
	if (acquire) {
		...
		barrier.dstAccessMask = read_accesses;
		...
		barrier.srcAccessMask = read_accesses;
	...
	vkCmdPipelineBarrier(cb->vk, src_stage, dst_stage, 0, 0, NULL, 0, NULL,
		1, &barrier);
	...
	wlr_log(WLR_DEBUG, "Vulkan readback: submitted %s command buffer "
		"timeline_point=%"PRIu64, acquire ? "acquire" : "release",
		timeline_point);
	return true;
}
...
enum wlr_vk_texture_readback_state waylib_vk_texture_begin_readback(
	...
	if (wlr_renderer == NULL || !wlr_renderer_is_vk(wlr_renderer)
			|| wlr_texture == NULL || !wlr_texture_is_vk(wlr_texture)) {
	...
	if (!texture->dmabuf_imported) {
		// Pixel (SHM upload) texture: CPU-written through our own staging
		// uploads, TRANSFER_SRC usage present, no foreign ownership cycle.
	...
	// The barrier below covers a single color aspect of one memory plane.
	if (texture->mem_count > 1 || vulkan_format_is_ycbcr(texture->format)) {
		wlr_log(WLR_ERROR, "Vulkan readback of multi-plane/YCbCr DMA-BUF "
			"textures is not supported");
		return WLR_VK_TEXTURE_READBACK_ERROR;
	}

	if (!submit_pending_stage_uploads(renderer)) {
		wlr_log(WLR_ERROR, "Failed to submit pending Vulkan staging before "
			"texture readback");
		return WLR_VK_TEXTURE_READBACK_ERROR;
	}
	...
	if (texture->buffer != NULL) {
		...
		if (!vulkan_sync_foreign_texture_acquire(texture, sync_file_fds)) {
			close_sync_file_fds(sync_file_fds);
			wlr_log(WLR_ERROR, "Failed to wait for foreign texture DMA-BUF "
				"fence before readback");
			return WLR_VK_TEXTURE_READBACK_ERROR;
		}
		if (!wait_sync_file_fds(sync_file_fds)) {
			wlr_log(WLR_ERROR, "Failed to wait for foreign texture DMA-BUF "
				"sync_file before readback");
			return WLR_VK_TEXTURE_READBACK_ERROR;
		}
	}

	if (!submit_readback_barrier(renderer, texture->image, true)) {
		wlr_log(WLR_ERROR, "Failed to submit Vulkan texture readback acquire");
		return WLR_VK_TEXTURE_READBACK_ERROR;
	}

	texture->readback_acquired = true;
	return WLR_VK_TEXTURE_READBACK_ACQUIRED;
}

void waylib_vk_texture_end_readback(struct wlr_renderer *wlr_renderer,
	...
	struct wlr_vk_renderer *renderer = vulkan_get_renderer(wlr_renderer);
	struct wlr_vk_texture *texture = vulkan_get_texture(wlr_texture);
	if (texture->renderer != renderer || !texture->readback_acquired) {
		return;
	}
	...
	texture->readback_acquired = false;
}
