...
public:
    ...
    void treeland_shortcut_manager_v2_commit_status(uint32_t success) override
    {
        qInfo() << "received commit status: " << success;
        emit commitStatusReceived();
    }

    void treeland_shortcut_manager_v2_activated(const QString &name) override
    {
        qInfo() << "shortcut activated: " << name;
    }
signals:
    ...
    QObject::connect(&manager, &ShortcutManagerV2::activeChanged, &manager, [&] {
        ...
        for (const QJsonValue &value : requests.array()) {
            ...
            if (req == "bind_key") {
                ...
                manager.bind_key(name, key, action);
