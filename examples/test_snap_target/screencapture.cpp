...
QList<ScreenCapture::CapturedOutput> ScreenCapture::captureOutputs()
{
    m_outputs.clear();
    ...
    for (Output *output : std::as_const(m_outputs)) {
        ...
        QByteArray name = QByteArrayLiteral("/treeland-snap-target-")
    ...
    for (Output *output : std::as_const(m_outputs)) {
        ...
        result.append({ QRect(output->logicalX, output->logicalY, w, h), output->image });
    }
