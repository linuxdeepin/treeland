...
RenderBufferBlitter {
    ...
    // The blitter only exists to feed the effect. When the effect is disabled
    // (or its content is rendered offscreen by the caller) there is nothing to
    // draw, so hide the blitter entirely: this also stops
    // WBufferRenderer::hasActiveRenderBufferBlitter() from requesting the
    // Vulkan backdrop and splitting the render pass for a disabled effect.
    visible: blitter.effectEnabled && !blitter.contentOffscreen

    // Keep the Vulkan effect implementation behind the Vulkan file selector.
    // This preserves the GLES2 Blur component and its lifecycle unchanged.
    Loader {
        id: contentLoader
        anchors.fill: parent
