...
public:
    ...
    void notifyTokenCommitted(const QString &token, struct ::wl_resource *tokenResource)
    {
        Q_EMIT q->tokenCommitted(token, tokenResource);
    }
...
void TokenContext::commit(Resource *resource)
{
    ...
    m_manager->notifyTokenCommitted(token, tokenResourceHandle());
