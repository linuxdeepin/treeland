...
void CaptureContextV1::onCapture(treeland_capture_frame_v1 *frame)
{
    ...
    auto notifyBuffer = [this] {
        ...
        if (!m_captureSource || !m_captureSource->imageValid()) {
            failFrame("without a valid source image");
            return;
        }

        const QRect crop = source()->cropRect();
        if (crop.isEmpty()) {
            failFrame("for an empty crop rect");
            return;
        }
    ...
    if (m_captureSource->imageValid()) {
        ...
        connect(m_captureSource,
                ...
                this,
                [this] {
                    failFrame("because the one-shot capture image could not be created");
                },
...
void CaptureContextV1::failFrame(const char *reason)
{
    if (!m_frame)
        return;

    qCWarning(lcTlCapture) << "Failing capture frame" << reason;
    m_frame->sendFailed();
    // The request is finished: drop the frame so a late imageReady cannot emit
    // a buffer after failed (the client is expected to destroy it now).
    m_frame = nullptr;
    if (m_captureSource)
        disconnect(m_captureSource, &CaptureSource::imageReady, this, nullptr);
}
...
void CaptureContextV1::handleFrameCopy(wlr_buffer *buffer)
{
    if (m_captureSource && m_frame) {
        if (!m_captureSource->copyBuffer(buffer)) {
            failFrame("because the client buffer copy failed");
            return;
        }
        m_frame->sendReady();
    } else {
...
bool CaptureSource::copyBuffer(wlr_buffer *buffer)
{
