...
namespace {
constexpr auto ddmRemoteUrl = "local:org.deepin.dde.ddm.qro";
}
...
void GreeterProxy::connectToDDM()
{
    ...
    if (!m_remoteNode->connectToNode(QUrl(QString::fromLatin1(ddmRemoteUrl)))) {
        qCCritical(treelandGreeter) << "Failed to connect to DDM remote node:" << ddmRemoteUrl;
        resetRemote();
    ...
    connect(m_remoteReplica.get(),
            ...
            this,
            &GreeterProxy::informationMessage);
    connect(m_remoteReplica.get(),
            ...
            [](QRemoteObjectPendingCallWatcher *watcher) {
                if (watcher->error() == QRemoteObjectPendingCall::NoError) {
                    sessionModel()->setSessions(
                        watcher->returnValue().value<QList<SessionEntry>>());
                } else {
