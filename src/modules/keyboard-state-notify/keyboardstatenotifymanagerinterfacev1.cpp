...
void TreelandKeyboardStateNotifyManagerInterfaceV1Private::setupKeyboardConnections()
{
    ...
    m_seatManagerSignals << QObject::connect(helper->seatManager(), &SeatManager::seatAdded, q, [this](WSeat *seat) {
        handleSeatAdded(seat);
    });
    m_seatManagerSignals << QObject::connect(helper->seatManager(), &SeatManager::seatRemoved, q, [this](WSeat *seat) {
