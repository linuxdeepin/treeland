...
{
    if (!surface || !wlr_surface_from_resource(surface)) {
        qCWarning(lcTlLayerShell) << "get_object: invalid surface resource";
        wl_resource_post_error(resource->handle,
                               error_wl_surface_invalid,
                               "surface resource is invalid!");
        return;
    }
                                                      ...
                                                      int32_t max_height)
{
    ...
    if (!seatClient || !seatClient->seat) {
        ...
        send_resize_rejected(resource->handle, resize_error_bad_seat);
        return;
    }
...
LayerShellExtensionObjectV1::~LayerShellExtensionObjectV1()
{
    ...
    WSeat *seat = d->resizingSeat;
    if (seat) {
        d->clearClamp();
        auto *container = Helper::instance()->rootSurfaceContainer();
        if (container)
            container->cancelMoveResizeForSeat(seat);
    }
}
