...
static void uninhibit(struct wl_client *client, struct wl_resource *resource, uint32_t id) {
    auto screensaver = static_cast<ScreensaverInterfaceV1 *>(wl_resource_get_user_data(resource));
    auto callback = wl_resource_create(client, &wl_callback_interface, 1, id);
    wl_callback_send_done(callback, screensaver->uninhibit(resource));
...
void ScreensaverInterfaceV1::inhibit(wl_resource *res, const char *appName, const char *reason) {
    m_inhibits.insert(res, std::make_tuple(appName, reason));
    Helper::instance()->updateIdleInhibitor();
}

uint32_t ScreensaverInterfaceV1::uninhibit(wl_resource *res) {
