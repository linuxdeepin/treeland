...
{
public:
    TreeLandForeignToplevelManagerV1Private(treeland_foreign_toplevel_manager_v1 *handle,
                                            bool isOwner,
                                            TreeLandForeignToplevelManagerV1 *qq)
        : QWObjectPrivate(handle, isOwner, qq)
    {
        ...
        sc.connect(&handle->events.destroy,
                   this,
                   &TreeLandForeignToplevelManagerV1Private::on_destroy);
        sc.connect(&handle->events.dock_preview_created,
                   this,
                   &TreeLandForeignToplevelManagerV1Private::on_dock_preview_created);
    }
...
{
    auto *context =
        TreeLandDockPreviewContextV1::from(static_cast<treeland_dock_preview_context_v1 *>(data));
    Q_EMIT q_func()->dockPreviewContextCreated(context);
}

TreeLandForeignToplevelManagerV1::TreeLandForeignToplevelManagerV1(
    treeland_foreign_toplevel_manager_v1 *handle, bool isOwner)
    : QObject(nullptr)
    , QWObject(*new TreeLandForeignToplevelManagerV1Private(handle, isOwner, this))
{
}

TreeLandForeignToplevelManagerV1 *TreeLandForeignToplevelManagerV1::get(
    treeland_foreign_toplevel_manager_v1 *handle)
{
    return TreeLandForeignToplevelManagerV1Private::map.value(handle);
}

TreeLandForeignToplevelManagerV1 *TreeLandForeignToplevelManagerV1::from(
    treeland_foreign_toplevel_manager_v1 *handle)
{
