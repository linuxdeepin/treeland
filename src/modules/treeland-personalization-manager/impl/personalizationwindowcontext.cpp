...
{
public:
    PersonalizationWindowContextPrivate(personalization_window_context_v1 *handle,
                                        bool isOwner,
                                        PersonalizationWindowContext *qq)
        : QWObjectPrivate(handle, isOwner, qq)
    {
        Q_ASSERT(!map.contains(handle));
        map.insert(handle, qq);
        sc.connect(&handle->events.destroy, this, &PersonalizationWindowContextPrivate::on_destroy);
        sc.connect(&handle->events.set_background_type,
                   this,
                   &PersonalizationWindowContextPrivate::on_set_background_type);
    }
...
PersonalizationWindowContext::PersonalizationWindowContext(
    personalization_window_context_v1 *handle, bool isOwner)
    : QObject(nullptr)
    , QWObject(*new PersonalizationWindowContextPrivate(handle, isOwner, this))
{
}

PersonalizationWindowContext *PersonalizationWindowContext::get(
    personalization_window_context_v1 *handle)
{
    return PersonalizationWindowContextPrivate::map.value(handle);
}

PersonalizationWindowContext *PersonalizationWindowContext::from(
    personalization_window_context_v1 *handle)
{
...
PersonalizationWindowContext *PersonalizationWindowContext::create(
    personalization_window_context_v1 *handle)
{
    if (!handle)
        return nullptr;
