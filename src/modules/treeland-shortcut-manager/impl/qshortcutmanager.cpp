...
{
public:
    QTreeLandShortcutManagerV1Private(treeland_shortcut_manager_v1 *handle,
                                      bool isOwner,
                                      QTreeLandShortcutManagerV1 *qq)
        : QWObjectPrivate(handle, isOwner, qq)
    {
        Q_ASSERT(!map.contains(handle));
        map.insert(handle, qq);
        sc.connect(&handle->events.destroy, this, &QTreeLandShortcutManagerV1Private::on_destroy);
        sc.connect(&handle->events.context,
                   this,
                   &QTreeLandShortcutManagerV1Private::on_new_context);
    }
...
QTreeLandShortcutManagerV1::QTreeLandShortcutManagerV1(treeland_shortcut_manager_v1 *handle,
                                                       bool isOwner)
    : QObject(nullptr)
...
{
public:
    QTreeLandShortcutContextV1Private(treeland_shortcut_context_v1 *handle,
                                      bool isOwner,
                                      QTreeLandShortcutContextV1 *qq)
        : QWObjectPrivate(handle, isOwner, qq)
    {
        Q_ASSERT(!map.contains(handle));
        map.insert(handle, qq);
...
QTreeLandShortcutContextV1::QTreeLandShortcutContextV1(treeland_shortcut_context_v1 *handle,
                                                       bool isOwner)
    : QObject(nullptr)
