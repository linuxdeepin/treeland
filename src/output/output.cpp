    ...
    , m_backlight(Backlight::createForOutput(output->output()))
{
    ...
    doSetBrightness(m_config->brightness());
    doSetColorTemperature(m_config->colorTemperature());

    connect(m_config, &OutputConfig::brightnessChanged, this, [this]() {
        doSetBrightness(m_config->brightness());
    });

    connect(m_config, &OutputConfig::colorTemperatureChanged, this, [this]() {
        doSetColorTemperature(m_config->colorTemperature());
    });
}
...
void Output::doSetBrightness(qreal brightness)
{
    ...
    if (m_backlight) {
        ...
        if (backlightBrightness != 0)
            brightnessCorrection = brightness / backlightBrightness;
    } else {
    ...
    generateGammaLUT(config()->colorTemperature(),
    ...
    screenViewport()->outputRenderWindow()->setOutputGammaLUT(screenViewport(), r, g, b);
}

// TODO: better Chromatic Adaptation algorithms can be implemented when the wlr_color_transform
// api is avalilable. For now RGB scaling is used due to limitation of gamma LUT table.
// see: http://www.brucelindbloom.com/index.html?ChromAdaptEval.html
void Output::doSetColorTemperature(uint32_t colorTemperature)
{
    ...
    qreal brightnessCorrection = config()->brightness();

    if (m_backlight && m_backlight->brightness() != 0) {
