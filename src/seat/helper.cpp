...
void Helper::init(Treeland::Treeland *treeland)
{
    ...
    m_shortcutManager = m_server->attach<ShortcutManagerV1>();
    connect(m_shortcutManager,
            &ShortcutManagerV1::requestCompositorAction,
            this,
...
bool Helper::beforeDisposeEvent(WSeat *seat, QWindow *, QInputEvent *event)
{
    ...
    if (!m_captureSelector && m_currentMode != CurrentMode::LockScreen
        ...
        do {
            ...
            auto user = m_userModel->currentUser();
            if (m_shortcutManager->dispatchKeySequence(user ? user->UID() : getuid(), sequence)) {
                return true;
            }
