...
SessionManager::~SessionManager()
{
    for (auto session : std::as_const(m_sessions)) {
        ...
        if (session->m_xwayland) {
            delete session->m_xwayland;
            session->m_xwayland = nullptr;
        }
        if (session->m_socket) {
 ...
 */
std::shared_ptr<Session> SessionManager::defaultSession() const
{
    ...
    auto createXWayland = [this](WSocket *socket) {
        ...
        connect(xwayland, &WXWayland::ready, this, [this, xwayland] {
            if (auto session = sessionForXWayland(xwayland)) {
                ...
                session->m_settingManagerThread = new QThread(session->m_xwayland);
                ...
                connect(session->m_settingManagerThread, &QThread::started, this, [session, scale, renderWindow]{
                    QMetaObject::invokeMethod(session->m_settingManager, [session, scale]() {
                            session->m_settingManager->setGlobalScale(scale);
                            session->m_settingManager->apply();
                        }, Qt::QueuedConnection);

                    QObject::connect(renderWindow,
                        &WOutputRenderWindow::effectiveDevicePixelRatioChanged,
                        session->m_settingManager,
                        [session](qreal dpr) {
                            session->m_settingManager->setGlobalScale(dpr);
                            session->m_settingManager->apply();
                        }, Qt::QueuedConnection);
                });
...
void SessionManager::updateActiveUserSession(const QString &username, int id)
{
    ...
    qCInfo(treelandCore) << "Listing on:" << session->m_socket->fullServerName();
}
