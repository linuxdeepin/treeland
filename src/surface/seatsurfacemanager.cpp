...
void SeatSurfaceManager::onKeyboardGrabEnd(wlr_seat_keyboard_grab *grab)
{
    ...
    // While showing the desktop, keyboard focus is on the desktop layer, not on
    // the (hidden) activated surface; do not yank it back to the window.
    const bool showingDesktop = Helper::instance()
        && Helper::instance()->showDesktopState() == ShowDesktopInterfaceV1::State::Show;

    if (popupStillFocused && !showingDesktop)
        setKeyboardFocusSurface(restoreTarget, Qt::ActiveWindowFocusReason);
