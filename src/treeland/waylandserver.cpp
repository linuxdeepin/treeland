...
static void context_handle_commit(struct wl_client *client, struct wl_resource *resource) {
    ...
    Q_EMIT context->manager->treeland->requestAddNewSocket(context->state.fd);
}

static const struct treeland_socket_context_v1_interface context_impl{
...
static struct treeland_socket_context_v1 *context_from_resource(struct wl_resource *resource) {
    assert(wl_resource_instance_of(resource, &treeland_socket_context_v1_interface, &context_impl));
                                           ...
                                           uint32_t id)
{
    ...
    wl_resource_set_implementation(resource, &context_impl, context, context_resource_destroy);

    wl_list_insert(&manager->contexts, &context->link);
}

static const struct treeland_socket_manager_v1_interface manager_impl{
    .create = manager_handle_create_listener,
};

static struct treeland_socket_manager_v1 *manager_from_resource(
        struct wl_resource *resource) {
    assert(wl_resource_instance_of(resource, &treeland_socket_manager_v1_interface, &manager_impl));
