...
{
    return getOutputId(output->output()->handle());
}
...
QMap<QString, TreelandWallpaperInterfaceV1::WallpaperType> WallpaperManager::globalValidWallpaper(wlr_output *excludedOutput)
{
...
QString WallpaperManager::currentWorkspaceWallpaper(WOutput *output)
{
    ...
    Q_ASSERT(workspace);
    for (int i = 0; i < m_wallpaperConfig.size(); ++i) {
        if (m_wallpaperConfig[i].outputName == getOutputId(output->handle())) {
...
void WallpaperManager::onImageChanged(int workspaceIndex, const QString &fileSource, TreelandWallpaperInterfaceV1::WallpaperRoles roles)
{
    ...
    QMap<QString, TreelandWallpaperInterfaceV1::WallpaperType> globalWallpapers = globalValidWallpaper();
    setOutputWallpaper(output->handle(),
                       workspaceIndex,
                       fileSource,
                       roles,
