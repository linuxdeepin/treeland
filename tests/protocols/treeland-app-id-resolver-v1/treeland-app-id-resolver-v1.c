...
/*
 * Server-side resolve state, mirrored from setup.cpp. The server thread
 * updates its private copy; the client reads g_app_id_resolver_snapshot,
 * which is copied on the server thread via protocol_test_invoke_server().
 */
...
/* Returns non-zero if the pidfd received over the wire refers to this process. */
...
/* Case 1: without a registered resolver the server refuses to start a resolve. */
...
/* Case 2: get_resolver creates a usable resolver object. */
...
/* Case 3: full resolve roundtrip — identify_request with a real pidfd, respond,
 * and the server-side callback receives the reported app id. */
...
/* Case 4: a second resolve gets the next request id; an empty app_id response
 * (failed resolution) is forwarded to the callback. */
...
/* Case 5 (last): a second get_resolver while a resolver is bound posts a
 * protocol error on the manager resource. Must run last: the error leaves the
 * connection in a permanent error state. */
static int duplicate_get_resolver_errors(struct test_ctx *ctx)
{
    /* This case roundtrips internally; the run loop must not roundtrip after it. */
    ctx->display_errored = 1;

    (void)treeland_app_id_resolver_manager_v1_get_resolver(ctx->manager);

    /* The roundtrip that processes the error returns -1 with errno EPROTO. */
    if (wl_display_roundtrip(ctx->display) >= 0)
        return 0;
...
int protocol_test_run(const char *socket_name)
{
    ...
    for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); ++i) {
        ...
        if (ctx.display_errored)
            continue; /* error case roundtripped internally; the connection is poisoned */
        if (wl_display_roundtrip(ctx.display) < 0)
