 ...
 *
 * The headless test fixture attaches only CaptureManagerV1 plus the compositor
 * globals from Treeland::initTestServer. There is no interactive capture
 * selector (QML CaptureSourceSelector / WOutputRenderWindow), so a capture
 * source can never become ready: the module refuses to create frame/session
 * objects without one and posts an implementation error ("Source is not
 * ready."). Those requests are therefore exercised on throwaway connections
 * and the observable protocol error is asserted instead of faking success.
 * The only client-driven server behavior that produces a real event is the
 * "selector busy" guard: while one context is in selection, a second
 * context's select_source is answered with source_failed(selector_busy).
 */
...
{
    /* First selection on an idle manager is accepted and becomes the
     * context in selection; no event is expected on this context. */
...
{
    /* A second context selecting while the first is still in selection must
     * be answered with source_failed(selector_busy). */
...
{
    /* Destroying the context that is in selection must clear the selection,
     * which is verified by the next case selecting without a busy failure. */
...
{
    /* capture() requires a ready capture source; without one the module posts
     * an implementation error on the display, killing the connection. Assert
     * the observable error rather than faking a frame. */
...
{
    /* Same guard as capture(): create_session without a ready source posts an
