 ...
 *
 * The treeland_ddm_v1 global is advertised at version 1 by DDMInterfaceV1
 * (InterfaceVersion = 1) even though the XML is version 2. Its request
 * handlers all dereference Helper::instance(), which only exists in the full
 * treeland QML boot, so they cannot be exercised in the headless fixture.
 * This test verifies the module's observable server state instead:
 *   - the advertised global version and version clamping at bind time,
 *   - DDMInterfaceV1::isConnected() across client bind/disconnect lifecycles,
 *   - that no unsolicited switch_to_vt / acquire_vt events are emitted.
 */
...
{
    /* The XML is version 2 (with destroy since=2), but DDMInterfaceV1
     * advertises InterfaceVersion 1, so the server must clamp to 1. */
...
static int second_client_keeps_connected(struct test_ctx *ctx)
{
    ...
    if (!aux_ddm)
        return 0;
    /* Roundtrip on the auxiliary client so the server has definitely bound it
     * before we query isConnected(). */
...
{
    /* Tear down the first client, then the second one. */
    if (ctx->ddm) {
    ...
    /* A fresh observer connection forces the server to process both closed
     * client sockets before we query isConnected(). */
...
{
    /* destroy is since=2 while the global is bound at v1, so the client-side
     * proxy destroy (no wire message) is the only correct teardown. */
    if (ctx->ddm)
        wl_proxy_destroy((struct wl_proxy *)ctx->ddm);
    protocol_test_disconnect(&ctx->checker);
    protocol_test_disconnect(&ctx->aux);
...
int protocol_test_run(const char *socket_name)
{
    ...
    for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); ++i) {
        ...
        /* The final case tears down the client connection, so the roundtrip
         * is only valid while a display is still open. */
