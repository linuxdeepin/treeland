 ...
 * Pure-C Wayland client for the treeland-personalization-manager-v1 protocol test.
 */
...
/* ---------- treeland_personalization_cursor_context_v1 events ---------- */
...
/* ---------- treeland_personalization_font_context_v1 events ---------- */
...
/* ---------- treeland_personalization_appearance_context_v1 events ---------- */
...
/* ---------- connection and object creation ---------- */
...
static int connect_client(struct test_ctx *ctx, const char *socket_name)
{
    ...
    /* The module advertises the manager global at version 1. */
...
/* ---------- window context requests (verified via server-side read-back) ---------- */
...
/* Empty theme: commit validation fails, so the first verfity event must be 0. */
...
void test_cleanup(struct test_ctx *ctx)
{
    ...
    /* The manager is bound at version 1; its destroy request is since version 2, so it is left to teardown. */
