...
/*
 * Snapshot of the server-side PersonalizationWindowContextV1 state, filled by
 * the C++ fixture (setup.cpp) when the client calls protocol_test_invoke_server
 * with personalization_window_state. The window context has no client events,
 * so the test verifies its requests through this read-back instead.
 */
...
struct test_ctx {
    ...
    /* invalid (empty-theme) cursor context events */
