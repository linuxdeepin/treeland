 ...
 * Pure-C Wayland client for the treeland-shortcut-manager-v2 protocol test.
 *
 * Notes on scope: the shortcut manager is per-session in treeland. The commit
 * path that actually applies bindings (and the activated / commit_failure
 * events) only runs when the compositor has an active session, which the
 * headless test server does not provide. This test therefore verifies the
 * session-independent contracts: binding the global, acquiring the manager,
 * the commit_success reply for an empty commit, the server-side buffering of
 * bind_key + commit without an active session, and the capture validation
 * failure (not_active) for an unfocused surface.
 */
...
{
    /* Without an active session the pending binding is buffered, not applied:
     * the first commit must not be answered with commit_success. */
...
{
    /* A fresh surface has no keyboard focus, so capture must fail immediately
     * with not_active and must not deliver a captured event. */
