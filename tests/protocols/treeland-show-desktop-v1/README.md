# `treeland-show-desktop-v1` 测试规范

## 范围

- 测试源码：`tests/protocols/treeland-show-desktop-v1/`、`tests/protocols/treeland-show-desktop-desktop-v1/`
...
| 场景 | 客户端发送 | 生产业务逻辑与断言 |
| --- | --- | --- |
| 初始/状态 API | 绑定并设置 show-desktop state | 初始事件与状态变化事件匹配生产 manager 状态 |
...
客户端发送 `set_show_desktop_state(SHOW)`。除等待 `show_desktop_state(SHOW)` 事件外，fixture 以
10ms 轮询、最多 2 秒等待生产可见性过渡完成，然后断言 `showDesktopState()==SHOW`、
`wrapper->isVisible()==false`、`wrapper->isMinimized()==false`，并读取 paint order 确认
该窗口不再参与渲染顺序。随后发送 `set_show_desktop_state(NORMAL)`，同样等待过渡完成并断言
...
仅覆盖一个 toplevel；多工作区、minimized policy 仍是独立场景。
