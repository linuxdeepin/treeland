                          ...
                          const char *name, struct wl_array *outputs)
{
    ...
    /* join the NUL-separated names with spaces for easy assertion */
    size_t used = 0;
...
{
    /* The module validates outputs->size (bytes), not the element count, so a
     * single-name array passes; an empty array (size 0) triggers
...
/* Fetching a name that was never created would make the server post a
 * protocol error (wl_resource_post_error) on the manager object. This path
 * is deliberately not exercised: this libwayland variant provides no
 * wl_display_add_error_listener(), and the default error handler aborts the
 * whole test process when a protocol error is dispatched. */
