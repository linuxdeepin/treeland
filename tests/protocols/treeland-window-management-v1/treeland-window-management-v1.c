 ...
 * Pure-C Wayland client for the treeland-window-management-v1 protocol test.
 */
...
{
    /* bind_resource sends the current state (Normal) on bind; it is delivered
     * by the first roundtrip. */
...
/* Deliberate server-side stimulus: no client request covers a compositor-side
 * state change, so drive the module's public setDesktopState() and observe the
 * broadcast show_desktop event. */
...
{
    /* 1 initial (bind) + 3 from set_desktop + 1 from the server stimulus. */
