...
int protocol_test_run(const char *socket_name)
{
    ...
    /* 1. Connect and create xdg_toplevel, wait for wrapper */
    ...
    /* 2. Bind wine_window_state_manager */
    ...
    /* 3. get_window_state(toplevel) -> verify initial state_changed(state=0) */
    ...
    /* E-level: surface is visible before minimize */
    if (!read_state(&state) || !state.visible)
        goto failed;
    stage = 5;

    /* 4. Invoke server to minimize -> verify state_changed with MINIMIZED
     *    AND E-level: surfaceItem is no longer visible in the render tree */
    client.state_events = 0;
    client.last_state = 0;
    ...
    if (client.state_events < 1
        ...
        goto failed;
    /* E-level: surface is hidden from the render tree (QQuickItem::isVisible = false) */
    if (!read_state(&state) || state.minimized != 1 || state.visible != 0)
        goto failed;
    stage = 6;

    /* 5. set_attention(0, 0) -> verify state_changed with ATTENTION flag.
     *    Attention requires a non-activated surface; the minimized window is
     *    not activated, so setAttention succeeds here.
     *    E-level: surfaceItem stays invisible (minimized) but attention flag set. */
    client.state_events = 0;
    client.last_state = 0;
    ...
    /* 6. clear_attention() -> verify state_changed without attention flag
     *    E-level: attention cleared but surface still invisible */
    client.state_events = 0;
    client.last_state = 0;
    ...
    /* 7. unminimize() -> verify state_changed without MINIMIZED
     *    E-level: surface returns to the render tree (QQuickItem::isVisible = true) */
    client.state_events = 0;
    client.last_state = 0;
    ...
    if (client.state_events < 1
        || (client.last_state & TREELAND_WINE_WINDOW_STATE_V1_STATE_MINIMIZED))
        goto failed;
    /* E-level: surface is visible again in the render tree */
