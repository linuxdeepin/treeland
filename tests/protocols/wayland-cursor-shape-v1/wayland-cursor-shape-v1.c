...
static int read_server(struct cursor_shape_server_state *state) {
	...
	if (invoke_on_server_thread(cursor_shape_read_server_state, state) == 0) {
		TEST_ERROR("cursor-shape: failed to read server state\n");
		return 1;
	}
	return 0;
}
...
int protocol_test_run(const char *socket_name) {
	...
	if (seat == NULL) {
		TEST_ERROR("cursor-shape: no wl_seat global\n");
	...
	if (!(g_caps & 0x1 /* WL_SEAT_CAPABILITY_POINTER */)) {
		TEST_ERROR("cursor-shape: seat has no pointer capability\n");
	...
	if (pointer == NULL) {
		TEST_ERROR("cursor-shape: wl_seat_get_pointer returned NULL\n");
		wl_seat_destroy(seat);
		client_disconnect(&conn);
		return 1;
	}
