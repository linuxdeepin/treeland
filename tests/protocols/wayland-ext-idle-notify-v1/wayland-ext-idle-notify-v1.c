...
int protocol_test_run(const char *socket_name) {
	...
	struct ext_idle_notifier_v1 *notifier =
		client_bind(&conn, ext_idle_notifier_v1_interface.name, &ext_idle_notifier_v1_interface, ext_idle_notifier_v1_interface.version);
	if (notifier == NULL) {
		TEST_ERROR("ext-idle-notify: failed to bind notifier: %s\n", strerror(errno));
	...
	if (n1 == NULL) {
		TEST_ERROR("ext-idle-notify: get_idle_notification returned NULL\n");
	...
	if (!wait_for_idled(&conn, &s1, 2000)) {
		TEST_ERROR("ext-idle-notify: Step 1 - idled did not fire\n");
		failed = 1;
	}
	...
	if (!xdg_toplevel_client_create_pending(&conn, &tc)) {
		TEST_ERROR("ext-idle-notify: create_pending failed\n");
		failed = 1;
		goto cleanup;
	}
	if (!xdg_toplevel_client_complete_map(&conn, &tc)) {
		TEST_ERROR("ext-idle-notify: complete_map failed\n");
	...
	if (inhibitor == NULL) {
		TEST_ERROR("ext-idle-notify: failed to create inhibitor\n");
		xdg_toplevel_client_destroy(&tc);
		failed = 1;
		goto cleanup;
	}
	...
	if (wait_for_idled(&conn, &s2, 500)) {
		TEST_ERROR("ext-idle-notify: Step 2 - idled fired while inhibited!\n");
		failed = 1;
	}
	...
	if (!wait_for_idled(&conn, &s3, 2000)) {
		TEST_ERROR("ext-idle-notify: Step 3 - idled did not fire after destroy\n");
		failed = 1;
	}
