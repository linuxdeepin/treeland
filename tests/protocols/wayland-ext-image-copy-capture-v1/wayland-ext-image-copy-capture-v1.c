...
int protocol_test_run(const char *socket_name)
{
    ...
    if (!client_connect(&conn, socket_name)) {
        fprintf(stderr, "ext-image-copy-capture: connect failed\n");
        return 1;
    }

    struct ext_output_image_capture_source_manager_v1 *src_mgr = client_bind(
        &conn, "ext_output_image_capture_source_manager_v1",
        &ext_output_image_capture_source_manager_v1_interface, 1);
    struct ext_image_copy_capture_manager_v1 *cap_mgr = client_bind(
        &conn, "ext_image_copy_capture_manager_v1",
        &ext_image_copy_capture_manager_v1_interface, 1);
    if (!src_mgr || !cap_mgr) {
        fprintf(stderr, "ext-image-copy-capture: failed to bind managers\n");
