...
static int wait_for_lock_result(struct client_connection *conn,
		struct lock_state *state)
{
	...
	while (!state->locked && !state->finished) {
		...
		const int elapsed = (now.tv_sec - start.tv_sec) * 1000
		...
		if (ready <= 0)
			return 0;
		if (wl_display_dispatch(conn->display) < 0)
			return 0;
	}

	return state->locked || state->finished;
}
...
int protocol_test_run(const char *socket_name) {
	...
	struct ext_session_lock_manager_v1 *manager = client_bind(
		&conn, ext_session_lock_manager_v1_interface.name, &ext_session_lock_manager_v1_interface, ext_session_lock_manager_v1_interface.version);
	if (manager == NULL) {
	...
	/* Treeland uses a 300 ms grace timer (Helper::onExtSessionLock).  Wait for
	 * the actual protocol event instead of polling with fixed sleeps. */
	const int got_lock_result = wait_for_lock_result(&conn, &state);
	...
	if (!got_lock_result) {
		...
		struct session_lock_server_state srv = { 0 };
