...
int protocol_test_run(const char *socket_name) {
	...
	if (!client_connect(&conn, socket_name)) {
		TEST_ERROR("fractional-scale: connect failed\n");
		return 1;
	}

	struct xdg_toplevel_client tc;
	if (!xdg_toplevel_client_create_with_solid_buffer(&conn, &tc, 64, 64, 0xffff0000u)) {
		TEST_ERROR("fractional-scale: create toplevel failed\n");
		client_disconnect(&conn);
		return 1;
	}

	struct wp_fractional_scale_manager_v1 *manager = client_bind(
		&conn, wp_fractional_scale_manager_v1_interface.name, &wp_fractional_scale_manager_v1_interface, wp_fractional_scale_manager_v1_interface.version);
	if (manager == NULL) {
		TEST_ERROR("fractional-scale: failed to bind manager: %s\n", strerror(errno));
		xdg_toplevel_client_destroy(&tc);
		client_disconnect(&conn);
		return 1;
	}

	struct fs_state state = { 0 };
	struct wp_fractional_scale_v1 *fs =
		wp_fractional_scale_manager_v1_get_fractional_scale(manager, tc.surface);
	if (fs == NULL) {
		TEST_ERROR("fractional-scale: get_fractional_scale returned NULL\n");
	...
	if (state.preferred_scale_count < 1) {
		TEST_ERROR("fractional-scale: no preferred_scale event received\n");
		failed = 1;
	} else if (state.scale == 0) {
		TEST_ERROR("fractional-scale: preferred_scale reported 0\n");
		failed = 1;
	}

	/* E-level: cross-check the preferred_scale against the live WOutput scale. */
	if (!failed) {
		struct fractional_scale_server_state srv = { 0 };
		if (invoke_on_server_thread(fractional_scale_read_server_state, &srv) == 0) {
			TEST_ERROR("fractional-scale: failed to read server state\n");
			failed = 1;
		} else if (!srv.valid) {
			TEST_ERROR("fractional-scale: no WOutput found\n");
			failed = 1;
		} else {
			uint32_t expected = (uint32_t)(srv.scale * 120.0f + 0.5f);
			if (expected != state.scale) {
				TEST_ERROR("fractional-scale: preferred_scale=%u but WOutput scale=%.2f\n"
					" (expected %u)",
