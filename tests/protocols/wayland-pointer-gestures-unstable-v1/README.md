...
`wl_client_get_credentials` 以与协议测试进程相同的 PID 缩小范围，并通过其绑定的
`zwp_pointer_gestures_v1` resource 精确定位测试 client；再读取该 resource 的
`user_data`（即真实的 `wlr_pointer_gestures_v1*` 句柄）。回读时仅统计该 client
拥有的 `swipes` 和 `pinches` 资源，避免同进程内部 client、Xwayland 或其他 client 的资源造成误报。
