...
static int read_server(struct pointer_gestures_server_state *state) {
	...
	if (invoke_on_server_thread(pointer_gestures_read_server_state, state) == 0) {
		TEST_ERROR("pointer-gestures: failed to read server state\n");
		return 1;
	}
	return 0;
}
...
int protocol_test_run(const char *socket_name) {
	...
	if (seat == NULL) {
		TEST_ERROR("pointer-gestures: no wl_seat global\n");
	...
	if (!(g_caps & 0x1)) {
		TEST_ERROR("pointer-gestures: seat has no pointer capability\n");
	...
	if (pointer == NULL) {
		TEST_ERROR("pointer-gestures: wl_seat_get_pointer returned NULL\n");
		wl_seat_destroy(seat);
		client_disconnect(&conn);
		return 1;
	}
