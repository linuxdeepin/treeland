...
static int read_server(struct primary_selection_server_state *state) {
	return TEST_READ_SERVER(primary_selection_read_server_state, state,
		"primary-selection: failed to read server state");
}

int protocol_test_run(const char *socket_name) {
	struct client_connection conn;
	if (!client_connect(&conn, socket_name)) {
		TEST_ERROR("primary-selection: connect failed\n");
		return 1;
	}

	struct client_seat client_seat = { 0 };
	if (!client_bind_seat(&conn, wl_seat_interface.version, &client_seat)) {
		TEST_ERROR("primary-selection: no wl_seat global\n");
		client_disconnect(&conn);
		return 1;
	}
	struct wl_seat *seat = client_seat.seat;
	if (!(client_seat.capabilities & WL_SEAT_CAPABILITY_KEYBOARD)) {
		TEST_ERROR("primary-selection: wl_seat has no keyboard capability\n");
		client_seat_destroy(&client_seat);
		client_disconnect(&conn);
		return 1;
	}
	...
	if (!xdg_toplevel_client_create(&conn, &tc)) {
		TEST_ERROR("primary-selection: toplevel create failed\n");
		if (kb != NULL)
	...
	if (g_enter_serial == 0) {
		TEST_ERROR("primary-selection: no keyboard enter serial\n");
	...
	struct zwp_primary_selection_device_manager_v1 *manager =
		client_bind(&conn, zwp_primary_selection_device_manager_v1_interface.name, &zwp_primary_selection_device_manager_v1_interface, zwp_primary_selection_device_manager_v1_interface.version);
	if (manager == NULL) {
		TEST_ERROR("primary-selection: failed to bind manager: %s\n", strerror(errno));
		xdg_toplevel_client_destroy(&tc);
		if (kb != NULL)
			wl_keyboard_destroy(kb);
		wl_seat_destroy(seat);
	...
	if (device == NULL) {
		TEST_ERROR("primary-selection: get_device returned NULL\n");
	...
	if (source == NULL) {
		TEST_ERROR("primary-selection: create_source returned NULL\n");
	...
	if (read_server(&srv)) {
		...
		TEST_ERROR("primary-selection: seat not found\n");
		failed = 1;
	} else if (!srv.has_source) {
		TEST_ERROR("primary-selection: primary_selection_source is NULL\n");
		failed = 1;
	}
