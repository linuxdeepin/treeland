...
static int read_server(struct relative_pointer_server_state *state) {
	...
	if (invoke_on_server_thread(relative_pointer_read_server_state, state) == 0) {
		TEST_ERROR("relative-pointer: failed to read server state\n");
		return 1;
	}
	return 0;
}
...
int protocol_test_run(const char *socket_name) {
	...
	if (seat == NULL) {
		TEST_ERROR("relative-pointer: no wl_seat global\n");
	...
	if (!(g_caps & 0x1)) {
		TEST_ERROR("relative-pointer: seat has no pointer capability\n");
	...
	if (pointer == NULL) {
		TEST_ERROR("relative-pointer: wl_seat_get_pointer returned NULL\n");
		wl_seat_destroy(seat);
		client_disconnect(&conn);
		return 1;
	}
