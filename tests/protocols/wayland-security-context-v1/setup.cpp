...
#include <protocols/private/wsecuritycontextmanager_p.h>
...
{
    (void)listener;
    const auto *event = static_cast<const Waylib::Server::wlr_security_context_v1_commit_event *>(data);
    g_committed = true;
...
void protocol_test_setup(Helper *helper)
{
    ...
    if (!mgr)
        return;
    auto *wlr_mgr = static_cast<Waylib::Server::wlr_security_context_manager_v1 *>(mgr->handle());
    Q_ASSERT(wlr_mgr);
    if (!wlr_mgr)
        return;
