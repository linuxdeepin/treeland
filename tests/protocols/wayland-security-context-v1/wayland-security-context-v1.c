...
int protocol_test_run(const char *socket_name) {
	...
	if (!client_connect(&conn, socket_name)) {
		TEST_ERROR("security-context: connect failed\n");
		return 1;
	}
	...
	if (listen_fd < 0) {
		TEST_ERROR("security-context: socket() failed\n");
	...
	if (bind(listen_fd, (struct sockaddr *)&addr, addrlen) < 0 || listen(listen_fd, 1) < 0) {
		TEST_ERROR("security-context: bind/listen failed\n");
		close(listen_fd);
		client_disconnect(&conn);
		return 1;
	}
	...
	if (socketpair(AF_UNIX, SOCK_STREAM, 0, close_fds) < 0) {
		TEST_ERROR("security-context: socketpair failed\n");
		close(listen_fd);
		client_disconnect(&conn);
		return 1;
	}
	...
	struct wp_security_context_manager_v1 *manager = client_bind(
		&conn, wp_security_context_manager_v1_interface.name, &wp_security_context_manager_v1_interface, wp_security_context_manager_v1_interface.version);
	if (manager == NULL) {
		TEST_ERROR("security-context: failed to bind manager: %s\n", strerror(errno));
		close(close_fds[0]);
		close(close_fds[1]);
		close(listen_fd);
		client_disconnect(&conn);
	...
	if (ctx == NULL) {
		TEST_ERROR("security-context: create_listener returned NULL\n");
		close(close_fds[0]);
		close(close_fds[1]);
		close(listen_fd);
		client_disconnect(&conn);
	...
	struct security_context_server_state srv = { 0 };
	...
	if (conn_fd < 0 || connect(conn_fd, (struct sockaddr *)&addr, addrlen) < 0) {
		TEST_ERROR("security-context: second connect failed\n");
		if (conn_fd >= 0)
			close(conn_fd);
	...
	if (sec_display == NULL) {
		TEST_ERROR("security-context: wl_display_connect_to_fd failed\n");
		wp_security_context_v1_destroy(ctx);
		wp_security_context_manager_v1_destroy(manager);
		close(close_fds[0]);
		close(close_fds[1]);
	...
	if (state.global_count < 1) {
		TEST_ERROR("security-context: no globals on second client\n");
		failed = 1;
	} else if (!state.has_compositor) {
		TEST_ERROR("security-context: wl_compositor not advertised\n");
		failed = 1;
	}

	/* E-level: verify the production commit captured the correct app_id. */
	if (!srv.valid) {
		TEST_ERROR("security-context: no commit event captured\n");
		failed = 1;
	} else if (!srv.app_id_match) {
		TEST_ERROR("security-context: app_id mismatch\n");
		failed = 1;
	}
