...
//
// Test the wp_security_context_manager_v1 global served by Treeland
// (wlr_security_context_manager_v1_create).  The client creates an abstract
// Unix listening socket + a close fd, commits a security context on them, then
// connects a second Wayland client through the listening socket and asserts the
// compositor accepts it and advertises globals — exercising the real
// security-context connection path.
...
static void sec_global(void *data, struct wl_registry *registry,
...
static void sec_global_remove(void *data, struct wl_registry *registry,
                              uint32_t name)
{
...
int protocol_test_run(const char *socket_name)
{
    ...
    if (!client_connect(&conn, socket_name)) {
        fprintf(stderr, "security-context: connect failed\n");
        return 1;
    }
    ...
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, close_fds) < 0) {
        fprintf(stderr, "security-context: socketpair failed\n");
        close(listen_fd);
        client_disconnect(&conn);
        return 1;
    }
    ...
    struct wp_security_context_manager_v1 *manager = client_bind(
        &conn, "wp_security_context_manager_v1",
        &wp_security_context_manager_v1_interface, 1);
    if (!manager) {
        fprintf(stderr, "security-context: failed to bind manager\n");
        close(close_fds[0]); close(close_fds[1]); close(listen_fd);
        client_disconnect(&conn);
        return 1;
    }
    ...
    wp_security_context_v1_set_app_id(ctx, "treeland-sec-ctx-test");
    ...
    if (conn_fd < 0 ||
        connect(conn_fd, (struct sockaddr *)&addr, addrlen) < 0) {
        fprintf(stderr, "security-context: second connect failed\n");
    ...
    if (!sec_display) {
        fprintf(stderr, "security-context: wl_display_connect_to_fd failed\n");
        wp_security_context_v1_destroy(ctx);
        wp_security_context_manager_v1_destroy(manager);
        close(close_fds[0]); close(close_fds[1]); close(listen_fd);
        client_disconnect(&conn);
    ...
    if (state.global_count < 1) {
        fprintf(stderr, "security-context: no globals on second client\n");
        failed = 1;
    } else if (!state.has_compositor) {
        fprintf(stderr, "security-context: wl_compositor not advertised\n");
        failed = 1;
    }
