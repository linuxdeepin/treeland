...
static int read_server(struct single_pixel_buffer_server_state *state) {
	return TEST_READ_SERVER(single_pixel_buffer_read_server_state, state,
		"single-pixel-buffer: failed to read server state");
}

int protocol_test_run(const char *socket_name) {
	struct client_connection conn;
	if (!client_connect(&conn, socket_name)) {
		TEST_ERROR("single-pixel-buffer: connect failed\n");
		return 1;
	}

	struct wp_single_pixel_buffer_manager_v1 *manager = client_bind(&conn,
		wp_single_pixel_buffer_manager_v1_interface.name, &wp_single_pixel_buffer_manager_v1_interface, wp_single_pixel_buffer_manager_v1_interface.version);
	if (manager == NULL) {
		TEST_ERROR("single-pixel-buffer: failed to bind manager: %s\n", strerror(errno));
	...
	if (!xdg_toplevel_client_create_pending(&conn, &tc)) {
		TEST_ERROR("single-pixel-buffer: create toplevel pending failed\n");
	...
	if (!failed) {
		buffer = wp_single_pixel_buffer_manager_v1_create_u32_rgba_buffer(
			manager, 0xffff0000u, 0, 0, 0xffffffffu);
		if (buffer == NULL) {
			TEST_ERROR("single-pixel-buffer: create_u32_rgba_buffer returned NULL\n");
			failed = 1;
		}
	}
	...
	if (!failed) {
		...
		if (read_server(&srv)) {
			...
			TEST_ERROR("single-pixel-buffer: no mapped SurfaceWrapper captured\n");
			failed = 1;
		} else if (!srv.mapped) {
			TEST_ERROR("single-pixel-buffer: WSurface not mapped\n");
			failed = 1;
		} else if (srv.buffer_width != 1 || srv.buffer_height != 1) {
			TEST_ERROR("single-pixel-buffer: expected 1×1 buffer, got %d×%d\n", srv.buffer_width, srv.buffer_height);
			failed = 1;
		}
	}
