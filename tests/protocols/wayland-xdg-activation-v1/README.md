...
| **生产回读** | — | 回读真实 `ActivationManagerInterfaceV1::activateRequested` 信号的 `disposition` == Attention | **E** |
...
捕获 `TokenDisposition`，回读断言其为 Attention。该 fixture 在创建 token 时未提供
serial，且目标 surface 尚未映射，因此对应 Treeland 的 Attention 分支，证明激活请求到达生产激活管道。

## 已知边界 / 下一项结果

仅验证该无 serial 场景的 Attention disposition；未验证携带 serial 的 Active 分支与焦点切换行为。
