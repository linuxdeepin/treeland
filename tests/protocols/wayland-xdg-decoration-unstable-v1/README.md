...
| 设置模式 | `set_mode(Client)` | 收到 `configure` 事件 | P |
...
客户端 `set_mode(Client)` 后，先断言收到 `configure` 事件；该事件中的 mode 是合成器的最终选择，不必等于客户端偏好。setup 再通过 `WXdgDecorationManager::modeBySurface`
