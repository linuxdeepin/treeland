...
static int read_modal(struct xdg_dialog_server_state *state) {
	...
	if (invoke_on_server_thread(xdg_dialog_read_server_state, state) == 0) {
		TEST_ERROR("xdg-dialog: failed to read server state\n");
		return 1;
	}
	return 0;
}
...
int protocol_test_run(const char *socket_name) {
	...
	if (!client_connect(&conn, socket_name)) {
		TEST_ERROR("xdg-dialog: connect failed\n");
		return 1;
	}
	...
	if (!xdg_toplevel_client_create_pending(&conn, &tc)) {
		TEST_ERROR("xdg-dialog: create_pending failed\n");
		client_disconnect(&conn);
		return 1;
	}
	if (!xdg_toplevel_client_complete_map(&conn, &tc)) {
		TEST_ERROR("xdg-dialog: complete_map failed\n");
	...
	if (!before.valid) {
		TEST_ERROR("xdg-dialog: no mapped SurfaceWrapper captured\n");
		failed = 1;
	} else if (before.modal) {
		TEST_ERROR("xdg-dialog: wrapper already modal before set_modal\n");
		failed = 1;
	}
	...
	if (!failed) {
		manager = client_bind(&conn, xdg_wm_dialog_v1_interface.name, &xdg_wm_dialog_v1_interface, xdg_wm_dialog_v1_interface.version);
		if (manager == NULL) {
			TEST_ERROR("xdg-dialog: failed to bind xdg_wm_dialog_v1: %s\n", strerror(errno));
			failed = 1;
		}
	}
	if (!failed) {
		...
		if (dialog == NULL) {
			TEST_ERROR("xdg-dialog: get_xdg_dialog returned NULL\n");
			failed = 1;
		}
	}
	if (!failed) {
		...
		if (read_modal(&after)) {
			...
			TEST_ERROR("xdg-dialog: SurfaceWrapper::modal() is false after set_modal\n");
			failed = 1;
		}
	}
