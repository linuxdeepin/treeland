...
static int read_server(struct xdg_foreign_v2_server_state *state) {
	return TEST_READ_SERVER(xdg_foreign_v2_read_server_state, state,
		"xdg-foreign-v2: failed to read server state");
}

int protocol_test_run(const char *socket_name) {
	struct client_connection conn;
	if (!client_connect(&conn, socket_name)) {
		TEST_ERROR("xdg-foreign-v2: connect failed\n");
		return 1;
	}
	...
	if (!xdg_toplevel_client_create(&conn, &tc_parent)) {
		TEST_ERROR("xdg-foreign-v2: failed to create parent toplevel\n");
	...
	if (!xdg_toplevel_client_create(&conn, &tc_child)) {
		TEST_ERROR("xdg-foreign-v2: failed to create child toplevel\n");
	...
	struct zxdg_exporter_v2 *exporter =
		client_bind(&conn, zxdg_exporter_v2_interface.name, &zxdg_exporter_v2_interface, zxdg_exporter_v2_interface.version);
	struct zxdg_importer_v2 *importer =
		client_bind(&conn, zxdg_importer_v2_interface.name, &zxdg_importer_v2_interface, zxdg_importer_v2_interface.version);
	if (exporter == NULL || importer == NULL) {
		TEST_ERROR("xdg-foreign-v2: failed to bind exporter/importer: %s\n", strerror(errno));
	...
	if (exported == NULL) {
		TEST_ERROR("xdg-foreign-v2: export_toplevel returned NULL\n");
		xdg_toplevel_client_destroy(&tc_child);
		xdg_toplevel_client_destroy(&tc_parent);
		client_disconnect(&conn);
		return 1;
	...
	if (!state.handle_received || state.handle[0] == '\0') {
		TEST_ERROR("xdg-foreign-v2: no handle event / empty handle\n");
		failed = 1;
	} else {
		...
		if (imported == NULL) {
			TEST_ERROR("xdg-foreign-v2: import_toplevel returned NULL\n");
			failed = 1;
		} else {
			...
			if (read_server(&srv)) {
				...
				TEST_ERROR("xdg-foreign-v2: SurfaceWrappers not captured\n");
				failed = 1;
			} else if (!srv.has_parent) {
				TEST_ERROR("xdg-foreign-v2: child toplevel has no parent in production\n");
				failed = 1;
			}
