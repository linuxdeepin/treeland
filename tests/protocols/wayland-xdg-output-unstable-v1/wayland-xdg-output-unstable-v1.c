...
int protocol_test_run(const char *socket_name) {
	 ...
	 * server-side read_server_state cross-checks via g_woutput. */
	struct wl_output *output = client_bind_last(&conn, wl_output_interface.name, &wl_output_interface, 2);
	if (output == NULL) {
		TEST_ERROR("xdg-output: no wl_output global\n");
	...
	int failed = 0;
	if (!state.done) {
		TEST_ERROR("xdg-output: no done event received\n");
		failed = 1;
