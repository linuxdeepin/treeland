...
struct KeyboardEventListeners {
    WInputDevice *device = nullptr;
    WSeatPrivate *seatPrivate = nullptr;
    WScopedListener keyListener;
...
public:
    ...
    WScopedListener m_requestSetCursorListener;
    WScopedListener m_requestSetSelectionListener;
    WScopedListener m_requestSetPrimarySelectionListener;
    WScopedListener m_requestStartDragListener;
    WScopedListener m_startDragListener;
    QList<KeyboardEventListeners*> m_keyboardListeners;
...
void WSeatPrivate::instantRelease()
{
    q_func()->m_handle = nullptr;
    m_requestSetCursorListener.remove();
    m_requestSetSelectionListener.remove();
    m_requestSetPrimarySelectionListener.remove();
    m_requestStartDragListener.remove();
    m_startDragListener.remove();
    for (auto *kel : std::as_const(m_keyboardListeners))
        delete kel;
...
void WSeatPrivate::connect()
{
    ...
    m_requestSetCursorListener.connect(&seat->events.request_set_cursor, [](wl_listener *listener, void *data) {
        ...
        self->on_request_set_cursor(static_cast<wlr_seat_pointer_request_set_cursor_event*>(data));
    });
    m_requestSetSelectionListener.connect(&seat->events.request_set_selection, [](wl_listener *listener, void *data) {
        auto *self = WScopedListener::owner<WSeatPrivate, &WSeatPrivate::m_requestSetSelectionListener>(listener);
        self->on_request_set_selection(static_cast<wlr_seat_request_set_selection_event*>(data));
    });
    m_requestSetPrimarySelectionListener.connect(&seat->events.request_set_primary_selection, [](wl_listener *listener, void *data) {
        auto *self = WScopedListener::owner<WSeatPrivate, &WSeatPrivate::m_requestSetPrimarySelectionListener>(listener);
        self->on_request_set_primary_selection(static_cast<wlr_seat_request_set_primary_selection_event*>(data));
    });
    m_requestStartDragListener.connect(&seat->events.request_start_drag, [](wl_listener *listener, void *data) {
        auto *self = WScopedListener::owner<WSeatPrivate, &WSeatPrivate::m_requestStartDragListener>(listener);
        self->on_request_start_drag(static_cast<wlr_seat_request_start_drag_event*>(data));
    });
    m_startDragListener.connect(&seat->events.start_drag, [](wl_listener *listener, void *data) {
        auto *self = WScopedListener::owner<WSeatPrivate, &WSeatPrivate::m_startDragListener>(listener);
        self->on_start_drag(static_cast<wlr_drag*>(data));
    });
}
...
void WSeatPrivate::attachInputDevice(WInputDevice *device)
{
    ...
    if (device->type() == WInputDevice::Type::Keyboard) {
        ...
        if (device == groupkeyboardDevice || device->isVirtual()) {
            ...
            auto *kel = new KeyboardEventListeners;
            kel->device = device;
            kel->seatPrivate = this;
            kel->keyListener.connect(&keyboard->events.key, [](wl_listener *listener, void *data) {
                auto *k = WScopedListener::owner<KeyboardEventListeners, &KeyboardEventListeners::keyListener>(listener);
                k->seatPrivate->on_keyboard_key(static_cast<wlr_keyboard_key_event*>(data), k->device);
            });
            kel->modifiersListener.connect(&keyboard->events.modifiers, [](wl_listener *listener, void *) {
                auto *k = WScopedListener::owner<KeyboardEventListeners, &KeyboardEventListeners::modifiersListener>(listener);
                k->seatPrivate->on_keyboard_modifiers(k->device);
            });
            m_keyboardListeners.append(kel);
...
void WSeatPrivate::detachInputDevice(WInputDevice *device)
{
    ...
    if (device->type() == WInputDevice::Type::Keyboard) {
        for (int i = m_keyboardListeners.size() - 1; i >= 0; --i) {
            if (m_keyboardListeners.at(i)->device == device) {
                delete m_keyboardListeners.takeAt(i);
            }
        }
...
WSeat *WSeat::fromHandle(const wlr_seat *handle)
{
    return static_cast<WSeat*>(WWrapObjectPrivate::fromNativeHandle(handle));
}
...
void WSeat::create(WServer *server)
{
    ...
    if (!qEnvironmentVariableIsSet("WAYLIB_DISABLE_GESTURE"))
        d->gesture = wlr_pointer_gestures_v1_create(server->nativeDisplay());
