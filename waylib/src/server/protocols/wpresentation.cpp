...
{
    // wlroots owns this global and destroys it from the display destroy
    // listener; null m_handle so handle()/global() return null instead of a
    // dangling pointer (kept as an explicit override rather than the inherited
    // empty base for this hardening).
    m_handle = nullptr;
}
