...
public:
    ...
    WWRAP_NATIVE_HANDLE_FUNCTIONS(wlr_xwayland_surface)
    ...
    WScopedListener m_associateListener;
    WScopedListener m_dissociateListener;
    WScopedListener m_setParentListener;
    WScopedListener m_requestActivateListener;
    WScopedListener m_requestConfigureListener;
    ...
    WScopedListener m_setOverrideRedirectListener;
    WScopedListener m_setGeometryListener;
    WScopedListener m_setHintsListener;
    WScopedListener m_setWindowTypeListener;
    WScopedListener m_setDecorationsListener;
    WScopedListener m_setTitleListener;
    WScopedListener m_setClassListener;
...
void WXWaylandSurfacePrivate::init()
{
    ...
    m_associateListener.connect(&handle()->events.associate, [this, q](wl_listener *, void *) {
    ...
    m_dissociateListener.connect(&handle()->events.dissociate, [this, q](wl_listener *, void *) {
        Q_ASSERT(surface);
    ...
    m_setParentListener.connect(&handle()->events.set_parent, [this](wl_listener *, void *) {
        updateParent();
    });
    m_requestActivateListener.connect(&handle()->events.request_activate, [q](wl_listener *, void *) {
        Q_EMIT q->requestActivate();
    });
    m_requestConfigureListener.connect(&handle()->events.request_configure,
                     [this, q] (wl_listener *, void *data) {
    ...
    m_requestFullscreenListener.connect(&handle()->events.request_fullscreen, [this, q](wl_listener *, void *) {
    ...
    m_requestMaximizeListener.connect(&handle()->events.request_maximize, [this, q](wl_listener *, void *) {
    ...
    m_requestMinimizeListener.connect(&handle()->events.request_minimize,
                     [q] (wl_listener *, void *data) {
    ...
    m_requestMoveListener.connect(&handle()->events.request_move,
                     [this, q] (wl_listener *, void *) {
        Q_EMIT q->requestMove(xwayland->seat(), 0);
    });
    m_requestResizeListener.connect(&handle()->events.request_resize,
                     [this, q] (wl_listener *, void *data) {
        auto *event = static_cast<wlr_xwayland_resize_event*>(data);
        Q_EMIT q->requestResize(xwayland->seat(), WTools::toQtEdge(event->edges), 0);
    });
    m_setOverrideRedirectListener.connect(&handle()->events.set_override_redirect,
                     [q] (wl_listener *, void *) {
        Q_EMIT q->bypassManagerChanged();
    });
    m_setGeometryListener.connect(&handle()->events.set_geometry,
                     [q] (wl_listener *, void *) {
        Q_EMIT q->geometryChanged();
    });
    m_setHintsListener.connect(&handle()->events.set_hints, [this](wl_listener *, void *) {
        updateSizeHints();
    });
    m_setWindowTypeListener.connect(&handle()->events.set_window_type,
                     [this] (wl_listener *, void *) {
                         updateWindowTypes();
                     });
    m_setDecorationsListener.connect(&handle()->events.set_decorations,
                     [q] (wl_listener *, void *) {
        Q_EMIT q->decorationsFlagsChanged();
    });
    m_setTitleListener.connect(&handle()->events.set_title,
                     [q] (wl_listener *, void *) {
        Q_EMIT q->titleChanged();
    });
    m_setClassListener.connect(&handle()->events.set_class,
                     [q] (wl_listener *, void *) {
...
{
    return static_cast<WXWaylandSurface*>(WWrapObjectPrivate::fromNativeHandle(handle));
}
...
{
    W_DC(WXWaylandSurface);
    wlr_xwayland_surface_configure(d_func()->handle(), d->handle()->x, d->handle()->y, size.width(), size.height());
}

void WXWaylandSurface::configure(const QRect &geometry)
{
    wlr_xwayland_surface_configure(d_func()->handle(), geometry.x(), geometry.y(), geometry.width(), geometry.height());
}
...
void WXWaylandSurface::setMaximize(bool on)
{
    ...
    d->maximized = on;
    wlr_xwayland_surface_set_maximized(d_func()->handle(), on, on);
...
void WXWaylandSurface::setMinimize(bool on)
{
    ...
    d->minimized = on;
    wlr_xwayland_surface_set_minimized(d_func()->handle(), on);
...
void WXWaylandSurface::setFullScreen(bool on)
{
    ...
    d->fullscreen = on;
    wlr_xwayland_surface_set_fullscreen(d_func()->handle(), on);
...
void WXWaylandSurface::setActivate(bool on)
{
    ...
    d->activated = on;
    wlr_xwayland_surface_activate(d_func()->handle(), on);
    Q_EMIT activateChanged();
}

void WXWaylandSurface::close()
{
    wlr_xwayland_surface_close(d_func()->handle());
}

void WXWaylandSurface::restack(WXWaylandSurface *sibling, StackMode mode)
{
    if (sibling) {
        wlr_xwayland_surface_restack(d_func()->handle(), sibling->handle(), static_cast<xcb_stack_mode_t>(mode));
        return;
    }

    wlr_xwayland_surface_restack(d_func()->handle(), nullptr, static_cast<xcb_stack_mode_t>(mode));
}
