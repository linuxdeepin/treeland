...
protected:
    wlr_buffer *beginRender(const QSize &pixelSize,
...
private:
    ...
    struct RenderState {
        ...
        WPixmanRegion dirty;
        // Cumulative buffer-space regions across sequential render sources.
        // Repair (renderRegion) is deliberately separate from content (flushRegion).
        WPixmanRegion flushRegion;
        WPixmanRegion renderRegion;
        bool needsRotateBuffer = false;
    } state;
    ...
    struct Data {
        ...
        // Owns the renderer, not its Qt effect QSGRootNode (a borrowed view).
        // The batch renderer owns its independent damage-tree root.
        QSGRenderer *renderer = nullptr;
    };
