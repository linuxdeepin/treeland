...
public:
    ...
    void prepare() override {
        ...
        if (isVulkan) {
            ...
            // The backdrop targets are created without a depth attachment (see
            // WRenderHelperPrivate::ensureVulkanBackdropRenderTargets): resuming
            // the outer pass without depth is only valid while the surrounding
            // renderer has the 2D depth buffer disabled, which this node's own
            // rendering flags force (no DepthAwareRendering). Fail closed if
            // that ever stops holding.
            if (currentRenderer->currentBatchRenderer()
                && rendererUseDepthBuffer(currentRenderer->currentBatchRenderer())) {
                setVulkanUnavailable("the surrounding renderer uses the depth buffer", ct);
                return;
            }
            ...
            if (currentDesc.colorAttachmentCount() < 1
                ...
                // The backdrop set must stay depth-free on both sides, and the
                // resumed pass must preserve the color attachment.
                || currentDesc.depthTexture()
                || resumeDesc.depthTexture()
...
private:
    ...
    void renderVulkan(QRhiTexture *currentTexture, QRhiTexture *backdropTexture) {
        ...
        // Nothing consumes the backdrop snapshot when there is no content to
        // draw, so the canvas copy and the pass split can be skipped entirely.
        if (!contentNode)
            return;
        ...
        // then resume on a render-pass-compatible target that loads the shared
        // color attachment (the backdrop set has no depth attachment).
        cb->endPass(rub);
    ...
    struct Node {
        ...
        ~Node() {
            if (clipNode) {
                // The clip node borrows the content subtree, so hand it back
                // before deleting it (exactly like the no-clip transition
                // below); QSGNode::destroy() then detaches it from rootNode.
                // Without this the clip node is leaked whenever the node is
                // destroyed while clipping was active.
                clipNode->reparentChildNodesTo(&rootNode);
                delete clipNode;
                clipNode = nullptr;
            }
        }
