...
- `WSGBatchRenderer` maps the output QSG tree onto `WSGDamageNode`s
  ...
  The compositor output pass runs `WSGDamageTracker::commit()`, which
  computes scene-content damage and accumulates it into registered
  viewports, populating each backdrop's per-output recopy debt. Each viewport
