...
#ifndef WSGBATCHRENDERER_P_H
#define WSGBATCHRENDERER_P_H
...
inline quint8 wsgNodeMutabilityGroup(const QSGNode *node)
{
#if QT_VERSION >= QT_VERSION_CHECK(6, 12, 0)
    return QSGNodePrivate::mutabilityGroup(node);
#else
    Q_UNUSED(node);
    return 0;
#endif
...
struct Element {
    ...
    void setNode(QSGGeometryNode *n) {
        ...
        mutabilityGroup = wsgNodeMutabilityGroup(n);
    }
...
public:
    ...
    static Renderer *create(QSGRenderContext *rc, QSGRendererInterface::RenderMode renderMode = QSGRendererInterface::RenderMode2D)
    {
        auto *drc = qobject_cast<QSGDefaultRenderContext *>(rc);
        Q_ASSERT(drc);
    ...
    void setDamage(const WDamageRegion &flush, const WDamageRegion &render,
                   const QRect &sceneOutputRect = {}, WSGViewport *viewport = nullptr);
...
private:
    ...
    {
        return m_damageTree != nullptr;
    }
    bool clipsDrawToDamage() const
    {
        return tracksDamage() && m_damageMode == DamageMode::Full;
    }
    void createDamageTree();
    void destroyDamageTree();
