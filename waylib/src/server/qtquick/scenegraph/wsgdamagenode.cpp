                                ...
                                WSGDamageTracker *tracker)
{
    ...
    if (m_needsBackdrop && hasContent()) {
        if (auto *backdrop = toBackdrop()) {
            ...
            if (!source.isEmpty()) {
                backdrop->accumulateRecopy(source.native(), tracker);
...
WSGDamageBackdropNode::~WSGDamageBackdropNode() = default;

void WSGDamageBackdropNode::accumulateRecopy(const pixman_region32_t *damage, WSGDamageTracker *tracker)
{
    m_recopySeed += damage;
    if (tracker)
        tracker->accumulateBackdropRecopy(this, damage);
}

const WPixmanRegion &WSGDamageBackdropNode::pendingRecopy(WSGViewport *viewport) const
{
    if (!viewport) {
    ...
    if (!data)
        return m_recopySeed;
    return data->pending(this);
}

void WSGDamageBackdropNode::consumeRecopy(const WPixmanRegion &applied, WSGViewport *viewport)
{
    if (applied.isEmpty() || !viewport)
        return;
    auto *data = viewport->getAttachedData<WSGViewportBackdropData>();
    if (!data)
        return;
    data->consume(this, applied);
}
