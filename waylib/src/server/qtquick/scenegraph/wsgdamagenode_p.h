...
protected:
    explicit WSGDamageNode(Type type);
    ...
    void updateWorld(const QMatrix4x4 &parentWorld,
                     ...
                     WSGDamageTracker *tracker = nullptr);
...
// into per-output recopy debt (scene space). One damage tree is shared by
// all outputs of a window, but each output owns its blit cache, so debt is
// stored per output and a renderer consumes only its own slot; unconsumed
// damage persists to later frames.
struct WSGViewportBackdropData
{
    struct DebtEntry {
        const WSGDamageBackdropNode *node = nullptr;
        WPixmanRegion damage;
    };
    QList<DebtEntry> debts;

    const WPixmanRegion &pending(const WSGDamageBackdropNode *node) const {
        for (const auto &entry : debts) {
            if (entry.node == node)
                return entry.damage;
        }
        static const WPixmanRegion empty;
        return empty;
    }

    void accumulate(const WSGDamageBackdropNode *node, const pixman_region32_t *damage) {
        for (auto &entry : debts) {
            if (entry.node == node) {
                entry.damage += damage;
                return;
            }
        }
        debts.append({node, WPixmanRegion(damage)});
    }

    void consume(const WSGDamageBackdropNode *node, const WPixmanRegion &applied) {
        for (auto &entry : debts) {
            if (entry.node == node) {
...
public:
    ...
    ~WSGDamageBackdropNode() override;

    const WPixmanRegion &pendingRecopy(WSGViewport *viewport) const;
    void accumulateRecopy(const pixman_region32_t *damage, WSGDamageTracker *tracker = nullptr);
    void consumeRecopy(const WPixmanRegion &applied, WSGViewport *viewport);
    ...
    const WPixmanRegion &recopySeed() const
    {
        return m_recopySeed;
    }
...
public:
    ...
    static const WPixmanRegion *pendingRecopy(const WSGDamageNode *n, WSGViewport *viewport)
    {
        if (const auto *backdrop = n->toBackdrop())
            return &backdrop->pendingRecopy(viewport);
        return nullptr;
    }
};
