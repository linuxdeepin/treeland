...
public:
    ...
    // Scene-space regions of this output's backdrop cache that are stale.
    // Lazily registers an unknown output with the accumulated history, so a
    // late-joining output conservatively recaptures everything stale so far
    // (cold outputs full-recapture anyway). Key identity is the driving
    // WBufferRenderer* passed as const void* to keep this header decoupled.
    const WPixmanRegion &pendingRecopy(const void *output) const;

    void accumulateRecopy(const pixman_region32_t *damage);

    // Renderer-side: drop the regions this output recaptured into its cache.
    void consumeRecopy(const WPixmanRegion &applied, const void *output);

    // Renderer-side: forget a destroyed output so the slot map stays bounded.
    void releaseRecopy(const void *output);
...
private:
    ...
    mutable QMap<const void *, WPixmanRegion> m_pendingRecopySlots;
...
public:
    ...
    static const WPixmanRegion *pendingRecopy(const WSGDamageNode *n, const void *output)
    {
        if (const auto *backdrop = n->toBackdrop())
            return &backdrop->pendingRecopy(output);
        return nullptr;
    }
};
