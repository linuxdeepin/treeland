...
void WSGImageNode::setDamageRegion(const WPixmanRegion &region)
{
    m_region = region;
    m_explicit = true;
...
void WSGImageNode::setOpaqueRegion(const WPixmanRegion &region)
{
    if (m_opaqueExplicit && m_opaque == region)
        return;
    m_opaque = region;
    m_opaqueExplicit = true;
    markDirty(QSGNode::DirtyMaterial);
}
