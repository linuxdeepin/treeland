...
// sanity check for Vulkan
    ...
    // Arm the per-QRhi leaked-resource safety sweep on every QRhi we adopt,
    // including windows whose render control never produces any BufferData:
    // client items holding Buffer/Texture/RenderBuffer objects that outlive
    // this QRhi would otherwise trip the VMA "Some allocations were not
    // freed" assertion at teardown (which was the Waydroid crash class).
    WRenderHelper::addVulkanTeardownSweep(rcd->rhi, rcd->rhi);
