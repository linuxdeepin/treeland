...
public:
    ...
    void addFrame(const WDamageRegion &damage, qint64 now = nowMs());
    void addFrame(const WPixmanRegion &content, bool full, qint64 now = nowMs())
    {
        addFrame(WDamageRegion(content, full), now);
    }
...
public:
    ...
    void addFrame(const WDamageRegion &damage);
    void addFrame(const WPixmanRegion &content, bool full)
    {
        addFrame(WDamageRegion(content, full));
    }
    void sync();
