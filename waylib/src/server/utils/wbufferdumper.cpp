                                                           ...
                                                           QImage &outputImage)
{
    ...
    uint32_t bytesPerPixel = QImage::toPixelFormat(qImageFormat).bitsPerPixel();
    uint32_t stride = texture->width * bytesPerPixel;
    outputImage = QImage(texture->width, texture->height, qImageFormat);
    if (outputImage.bytesPerLine() != static_cast<int>(stride)) {
        stride = outputImage.bytesPerLine();
    }

    wlr_texture_read_pixels_options options = {
        .data = outputImage.bits(),
        .format = format,
        .stride = stride,
    };
