...
static inline wlr_xcursor *getXCursorWithFallback(wlr_xcursor_manager *xcursorManager,
                                                  const char *name, float scale)
{
    if (!xcursorManager)
        return nullptr;

    if (wlr_xcursor *cursor = wlr_xcursor_manager_get_xcursor(xcursorManager, name, scale)) {
        return cursor;
    }
    ...
    for (const auto &alterName : alterNameList) {
        if (auto cursor = wlr_xcursor_manager_get_xcursor(
                xcursorManager, alterName.constData(), scale))
            return cursor;
    }
