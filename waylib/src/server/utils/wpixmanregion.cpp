...
// No allocation: pixman answers overlap with a rectangle containment query.
bool WPixmanRegion::intersects(const QRect &rect) const
{
    if (isEmpty() || rect.isEmpty())
        return false;
    pixman_box32_t box = { rect.x(), rect.y(), rect.x() + rect.width(),
                           rect.y() + rect.height() };
    return pixman_region32_contains_rectangle(&m_region, &box) != PIXMAN_REGION_OUT;
}
