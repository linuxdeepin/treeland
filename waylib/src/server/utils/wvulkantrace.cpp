...
std::unordered_map<quintptr, std::unique_ptr<WScopedListenerList>> s_sourceListeners;
std::unordered_map<quintptr, std::unique_ptr<WScopedListenerList>> s_clientListeners;
...
wlr_buffer *sourceBufferFor(wlr_buffer *clientBuffer, bool createWrapper)
{
    if (!clientBuffer)
        return nullptr;

    auto *client = wlr_client_buffer_get(clientBuffer);
    if (client) {
...
{
    std::unique_ptr<WScopedListenerList> listeners;
    {
        QMutexLocker locker(&s_mutex);
        s_clientBuffers.remove(key);
        auto it = s_clientListeners.find(key);
        if (it != s_clientListeners.end()) {
            listeners = std::move(it->second);
            s_clientListeners.erase(it);
        }
    }
}

void sourceReleased(quintptr sourceKey, wlr_buffer *sourceBuffer)
{
    QString message;
    bool candidate = false;
    {
        ...
        size_t lockCount = 0;
        if (sourceBuffer) {
            dropped = sourceBuffer->dropped;
            lockCount = sourceBuffer->n_locks;
        }
...
void sourceDestroyed(quintptr sourceKey)
{
    ...
    std::unique_ptr<WScopedListenerList> listeners;
    {
        ...
        auto listenerIt = s_sourceListeners.find(sourceKey);
        if (listenerIt != s_sourceListeners.end()) {
            listeners = std::move(listenerIt->second);
            s_sourceListeners.erase(listenerIt);
        }
    }
    if (tracked)
        qCDebug(lcWlQtQuickTexture).noquote() << message;
...
bool registerBuffers(wlr_buffer *clientBuffer, bool createSourceWrapper,
                     ...
                     quintptr *sourceKey)
{
    ...
    const quintptr clientKey = reinterpret_cast<quintptr>(clientBuffer);
    ...
    const bool isDmabuf = wlr_buffer_get_dmabuf(source, &dmabuf);
    const bool isShm = !isDmabuf && wlr_buffer_get_shm(source, &shm);

    const quintptr localSourceKey = reinterpret_cast<quintptr>(source);
    ...
    if (connectSourceSignals) {
        auto listeners = std::make_unique<WScopedListenerList>();
        listeners->add(&source->events.release,
                       [localSourceKey, source] (void *) {
            sourceReleased(localSourceKey, source);
        });
        listeners->add(&source->events.destroy, [localSourceKey] (void *) {
            sourceDestroyed(localSourceKey);
        });
        QMutexLocker locker(&s_mutex);
        s_sourceListeners.emplace(localSourceKey, std::move(listeners));
    }
    ...
    if (connectClientDestroy) {
        auto listeners = std::make_unique<WScopedListenerList>();
        listeners->add(&clientBuffer->events.destroy, [clientKey] (void *) {
            removeClientBuffer(clientKey);
        });
        QMutexLocker locker(&s_mutex);
        s_clientListeners.emplace(clientKey, std::move(listeners));
    }
...
void surfaceCommit(const void *surface, qint64 pid, quint64 surfaceSeq,
                   quint32 committedState, wlr_buffer *clientBuffer)
{
    ...
    quintptr texture = 0;
    if (auto *client = wlr_client_buffer_get(clientBuffer))
    ...
    qCDebug(lcWlSurface).noquote()
        << QStringLiteral("VKTRACE event=surface-commit surface=%1 pid=%2 surfaceSeq=%3 committed=%4 generation=%5 clientBufferId=%6 clientBuffer=%7 sourceBufferId=%8 sourceBuffer=%9 texture=%10 bufferType=%11")
               ...
               .arg(generation)
               .arg(clientId)
               .arg(pointerName(reinterpret_cast<quintptr>(clientBuffer)))
               .arg(sourceId)
               .arg(pointerName(sourceKey))
...
void providerBind(const void *provider, WOutputRenderWindow *window,
                  wlr_buffer *clientBuffer, wlr_texture *texture)
{
    ...
    const auto *ownerClientBuffer = wlr_client_buffer_get(clientBuffer);
    const char *ownerStatus = !ownerClientBuffer
        ? "provider-owned"
        : (ownerClientBuffer->texture == texture
               ? "matched"
               : "mismatch");
    ...
    qCDebug(lcWlQtQuickTexture).noquote()
        << QStringLiteral("VKTRACE event=provider-bind action=reject owner=%1 frame=%2 stage=%3 provider=%4 clientBuffer=%5 texture=%6")
               ...
               .arg(pointerName(clientBuffer
                                    ? reinterpret_cast<quintptr>(clientBuffer)
                                    : 0))
               .arg(pointerName(reinterpret_cast<quintptr>(texture)));
}

void providerDiscard(const void *provider, WOutputRenderWindow *window,
                     wlr_texture *texture, const char *reason)
{
    if (!enabled() || !provider || !window || !texture)
        return;
                              ...
                              wlr_buffer *clientBuffer, wlr_texture *texture)
{
    ...
    if (!token.sourceBufferId && clientBuffer) {
        QMutexLocker locker(&s_mutex);
        const auto client = s_clientBuffers.value(reinterpret_cast<quintptr>(clientBuffer));
...
void qtWrap(bool begin, wlr_texture *texture, quint64 image, quint32 vkFormat,
...
void beginPass(WOutputRenderWindow *window, const void *renderer,
               wlr_buffer *targetBuffer, const char *purpose,
               int sourceIndex, qsizetype providerCount)
{
    ...
    PassRecord pass;
    {
        ...
        pass.targetBuffer = reinterpret_cast<quintptr>(targetBuffer);
...
void sampleDisposition(WOutputRenderWindow *window, const void *provider,
                       wlr_texture *texture, SampleDisposition disposition,
                       qint64 elapsedUsec)
{
    if (!enabled() || !window)
        return;
...
void sampleFinished(WOutputRenderWindow *window, wlr_texture *texture,
...
void surfaceFootprint(WOutputRenderWindow *window, const void *provider,
                      wlr_texture *texture, const void *surface)
{
    if (!enabled() || !window)
        return;
...
void outputCommitState(wlr_output *output, quint32 committedState)
{
    if (!enabled() || !output)
        return;
    QMutexLocker locker(&s_mutex);
    auto &states = s_outputCommitStates[reinterpret_cast<quintptr>(output)];
    states.insert(output->commit_seq, committedState);
    while (states.size() > MaxTrackedOutputCommits)
        states.erase(states.begin());
}

void outputCommitted(WOutputRenderWindow *window, wlr_output *output,
                     quint32 sequenceBefore, bool ok)
{
    if (!enabled() || !window || !output)
        return;

    const quint32 sequenceAfter = output->commit_seq;
    ...
    bool relevant = false;
    {
        ...
        if (!relevant)
            return;
        if (sequenceAdvanced) {
            committedState = s_outputCommitStates.value(reinterpret_cast<quintptr>(output))
                                 .value(sequenceAfter);
            auto &commits = s_outputCommits[reinterpret_cast<quintptr>(output)];
    ...
    const QString message = QStringLiteral("VKTRACE event=output-commit frame=%1 output=%2 seqBefore=%3 commitSeq=%4 seqAdvanced=%5 result=%6 stateKnown=%7 committedState=%8 bufferCommitted=%9")
                                .arg(frameId)
                                .arg(pointerName(reinterpret_cast<quintptr>(output)))
                                .arg(sequenceBefore)
                                .arg(sequenceAfter)
...
void outputPresented(wlr_output *output, const wlr_output_event_present *event)
{
    if (!enabled() || !output || !event)
        return;
    ...
    {
        QMutexLocker locker(&s_mutex);
        auto outputIt = s_outputCommits.find(reinterpret_cast<quintptr>(output));
...
void outputDestroyed(wlr_output *output)
{
    if (!enabled() || !output)
        return;
    const quintptr outputKey = reinterpret_cast<quintptr>(output);
    QMutexLocker locker(&s_mutex);
