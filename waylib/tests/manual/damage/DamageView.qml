...
Page {
    ...
    anchors.fill: parent
    padding: 0
    background: Rectangle { color: "#0a0f1a" }
    readonly property color color: "#0a0f1a"
    ...
    readonly property int detectedRefreshRate: 60
    ...
    function calculateTreeDrop(mouseXInTree, mouseYInTree) {
        ...
        // Derive the stride from delegate geometry instead of hard-coding 38.
        let rowStride = 36 + tree.spacing
        if (tree.contentItem.children.length > 0)
            rowStride = tree.contentItem.children[0].height + tree.spacing
        const rowTop = index * rowStride - tree.contentY
        ...
        if (relY < 10) {
            ...
            treeDropMode = 3
            treeDropParentId = targetParentId
            // treeNodes is a pre-order flattening: the target's descendants
            // sit right behind it, so skip them to find the next sibling.
            const targetDepth = target.depth || 0
            let nextId = 0
            for (let j = index + 1; j < count; ++j) {
                const following = scene.treeNodes[j]
                if ((following.depth || 0) > targetDepth)
                    continue
                if ((following.parentId || 0) === targetParentId)
                    nextId = following.id
                break
            }
            treeDropBeforeId = nextId
    ...
    header: Rectangle {
        ...
        RowLayout {
            ...
            RowLayout {
                ...
                Label { text: "损伤高亮"; color: win.mutedColor; font.pixelSize: 10 }
                ComboBox {
                    id: damageModeBox
                    Layout.preferredWidth: 120
                    model: ["highlight", "off"]
                    Component.onCompleted: currentIndex = Math.max(0, model.indexOf(Helper.damageVisual))
                    onActivated: Helper.damageVisual = currentText
                    ToolTip.visible: hovered
                    ToolTip.text: "OutputRenderWindow highlight/off（WAYLIB_DEBUG_DAMAGE）"
                    Connections {
                        target: Helper
                        function onDamageDebugModeChanged() {
                            damageModeBox.currentIndex = Math.max(0, damageModeBox.model.indexOf(Helper.damageDebugMode))
                        }
                    }
                }
                                ...
                                onReleased: function(mouse) {
                                    if (mouse.button === Qt.LeftButton) {
                                        if (win.treeDragging) {
                                            ...
                                            // Single reset, single commit: the duplicated
                                            // resetTreeDrag/moveNode below double-committed drops.
                                            win.resetTreeDrag()
                                            endUserInteraction()
                                            if (hasValidDrop) {
            ...
            ColumnLayout {
                ...
                Rectangle {
                    ...
                    Item {
                        ...
                        // 真实损伤由 WSGBatchRenderer 高亮，模拟层默认隐藏
                        Rectangle {
                            anchors.fill: parent
                            color: "transparent"
                            visible: Helper.damageDebugMode === "highlight"
                            Text {
                                anchors.right: parent.right
                                anchors.bottom: parent.bottom
                                anchors.margins: 8
                                text: "WSG 真实高亮（WSGDamageDebug）"
                                color: "#66ffacba"
                                font.pixelSize: 9
                            }
                        }
