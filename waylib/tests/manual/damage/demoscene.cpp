...
static QVariantList regionToRects(const WPixmanRegion &region)
{
    QVariantList list;
    int count = 0;
    ...
    for (int i = 0; i < count && boxes; ++i) {
        QVariantMap m;
        m.insert(QStringLiteral("x"), boxes[i].x1);
        m.insert(QStringLiteral("y"), boxes[i].y1);
        m.insert(QStringLiteral("w"), boxes[i].x2 - boxes[i].x1);
        m.insert(QStringLiteral("h"), boxes[i].y2 - boxes[i].y1);
        list.append(m);
    }
    return list;
}
...
void DemoScene::resetRoot()
{
    ...
    m_tracker.addViewport(m_demoViewport);
    m_decor.clear();
    m_dragFrameTimer.stop();
    m_dragFramePending = false;
...
void DemoScene::setSelectedId(quint64 id)
{
    ...
    if (WSGDamageNode *n = findNode(id)) {
        if (auto *tr = n->toTransform()) {
            const QTransform m = tr->matrix().toTransform();
...
void DemoScene::setTranslationSelected(qreal x, qreal y)
{
    ...
    if (!n || !n->toTransform())
        return;
    QTransform matrix = n->toTransform()->matrix().toTransform();
...
void DemoScene::setRotationSelected(qreal degrees, int axis)
{
    ...
    m_rotationAxis = axis;
    m_rotation = degrees;
    const QTransform current = n->toTransform()->matrix().toTransform();
    QTransform matrix;
...
{
    // Single tracker commit, dual-clipped: one scene-space commit feeds both
    // simulated outputs; each view only clips the shared damage to its own
    // viewport rect for display.
    m_tracker.commit();
    WPixmanRegion damageA = m_demoViewport->damageRegion().region;
    WPixmanRegion damageB = damageA;
    m_tracker.finishFrame();
    m_demoViewport->finishFrame();
    ...
    const WPixmanRegion combined = damageA + damageB;
    if (!combined.isEmpty()) {
        QVariantMap frame;
...
void DemoScene::refreshSelectedProps()
{
    ...
    if (auto *tr = n->toTransform()) {
        const QTransform matrix = tr->matrix().toTransform();
