...
void Helper::setDamageVisual(const QString &mode)
{
    if (!m_window)
        return;
    const QString lower = mode.toLower().trimmed();
    WOutputRenderWindow::DamageVisual visual = WOutputRenderWindow::DamageVisual::Off;
    if (lower == "highlight" || lower == "1" || lower == "true" || lower == "on")
        visual = WOutputRenderWindow::DamageVisual::Highlight;
    else if (lower == "off" || lower == "none" || lower == "0" || lower == "false")
        visual = WOutputRenderWindow::DamageVisual::Off;
    else
        return;
    if (m_window->damageVisual() == visual)
        return;
    m_window->setDamageVisual(visual);
...
void Helper::initProtocols(WOutputRenderWindow *window, QQmlEngine *qmlEngine)
{
    ...
    connect(window, &WOutputRenderWindow::damageVisualChanged, this, [this] {
        Q_EMIT damageVisualChanged();
        Q_EMIT damageDebugModeChanged();
    });
    window->init(m_renderer, m_allocator);

    // Default highlight unless env already sets
    if (qEnvironmentVariableIsEmpty("WAYLIB_DEBUG_DAMAGE") && window->damageVisual() == WOutputRenderWindow::DamageVisual::Off) {
        window->setDamageVisual(WOutputRenderWindow::DamageVisual::Highlight);
    }
