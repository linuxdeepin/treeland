    ...
    while (created < totalNodes) {
        ...
        for (int i = 0; i < childrenInLayer; ++i) {
            if (i == 3 && outBackdrops && created + 1 < totalNodes) {
                // A real backdrop node (not a flagged geometry node) so the
                // tracker accumulates behind-damage and dilates it by the
                // 16px recopy expansion, like a blur blitter.
                ...
                bg->setRecopyExpansion(QMargins(16, 16, 16, 16));
                             ...
                             int)> &prepareIteration)
{
    ...
    std::vector<std::shared_ptr<WSGViewport>> viewports(viewportCount);
    for (int i = 0; i < viewportCount; ++i) {
        viewports[i] = std::make_shared<WSGViewport>();
        tracker.addViewport(viewports[i]);
    }

    auto cycle = [&] {
        // The first commit includes backdrop sampling and dilation; subsequent
        // output consumers reuse the same final scene damage.
        tracker.commit();
        for (int i = 0; i < viewportCount; ++i) {
            (void)viewports[i]->damageRegion();
            viewports[i]->finishFrame();
        }
        // iteration measures the same work and timing stays deterministic.
        for (auto *bd : backdrops) {
            if (auto *backdrop = bd->toBackdrop())
                backdrop->consumeRecopy(backdrop->pendingRecopy(viewports[0].get()), viewports[0].get());
        }
    };
...
void WSGDamageGraphBench::benchmarkSuite()
{
    ...
    printf(
        "| 场景分类 (Scenario) | 用例 (Case) | 节点数 (Nodes) | Viewport数 | 平均单帧耗时 (Avg Time) | "
        "最大理论吞吐量 (Throughput) | 60Hz 帧预算占比 | 144Hz 帧预算占比 | 240Hz 帧预算占比 |\n");
    printf("|---|---|---|---|---|---|---|---|---|\n");
    for (const auto &r : s_results) {
        printf("| %s | %s | %d | %d VP | **%.2f µs** | %.0f FPS | %.3f%% | %.3f%% | %.3f%% |\n",
               r.category.toUtf8().constData(),
               r.scenario.toUtf8().constData(),
               r.nodeCount,
               r.viewportCount,
               r.avgTimeUs,
               r.maxFps,
