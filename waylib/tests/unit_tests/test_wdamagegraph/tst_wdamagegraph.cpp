...
static QMatrix4x4 sceneToBuffer(const WSGViewport &v, const QSize &buf, qreal dpr = 1.0)
{
    QTransform t;
    const auto outputSize = QSizeF(buf) / dpr;
    const QRectF sourceRect = v.sourceRect();
    const QRectF targetRect = v.targetRect();
    ...
    QMatrix4x4 scale;
    scale.scale(dpr, dpr);
    return scale * (QMatrix4x4(t) * v.renderMatrix());
}
...
{
public:
    TestTracker(WSGDamageNode *root = nullptr)
        : WSGDamageTracker(root)
        , viewport(std::make_shared<WSGViewport>())
    {
    ...
    std::shared_ptr<WSGViewport> viewport;
};
...
void WSGDamageGraphTest::backdropRecopyClippedToSampleBounds()
{
    ...
    tracker.finishFrame();
    viewport->finishFrame();
    glass->consumeRecopy(glass->pendingRecopy(viewport.get()), viewport.get());
    ...
    const WPixmanRegion recopy = glass->pendingRecopy(viewport.get());
...
void WSGDamageGraphTest::stackedBackdropsIndependentCopySources()
{
    ...
    for (auto *glass : { left, overlap, right })
        glass->consumeRecopy(glass->pendingRecopy(tracker.viewport.get()), tracker.viewport.get());

    wall->markContentDirty(QRect(50, 50, 10, 10));
    tracker.commit();
    COMPARE_REGION(WPixmanRegion(*WSGDamageNodeTestAccess::pendingRecopy(left, tracker.viewport.get())),
                   QRegion(50, 50, 10, 10));
    COMPARE_REGION(WPixmanRegion(*WSGDamageNodeTestAccess::pendingRecopy(overlap, tracker.viewport.get())),
                   QRegion(50, 50, 10, 10));
    COMPARE_REGION(WPixmanRegion(*WSGDamageNodeTestAccess::pendingRecopy(right, tracker.viewport.get())),
                   QRegion());
...
void WSGDamageGraphTest::backdropSeesRemovedSiblingHole()
{
    ...
    TestTracker tracker(&root);
    tracker.commit();
    backdrop->consumeRecopy(backdrop->pendingRecopy(tracker.viewport.get()), tracker.viewport.get());

    group->removeChild(back);
    tracker.commit();
    COMPARE_REGION(WPixmanRegion(*WSGDamageNodeTestAccess::pendingRecopy(backdrop, tracker.viewport.get())),
                   QRegion(20, 20, 40, 40));
    COMPARE_REGION(tracker.commit(), QRegion());
}
...
void WSGDamageGraphTest::backgroundDamageRecopySurvivesRepeatedCommit()
{
    ...
    const WPixmanRegion recopyDebt = glass->pendingRecopy(viewport.get());
    ...
    tracker.finishFrame();
    viewport->finishFrame();
    glass->consumeRecopy(recopyDebt, viewport.get());
    ...
    QVERIFY(glass->pendingRecopy(viewport.get()).isEmpty());
}
...
void WSGDamageGraphTest::multiOutputDisjointSceneMapping()
{
    ...
    auto viewport1 = std::make_shared<WSGViewport>();
    viewport1->setRenderParameters(QMatrix4x4(), QRectF(0, 0, 1920, 1080));
    auto viewport2 = std::make_shared<WSGViewport>();
    viewport2->setRenderParameters(QMatrix4x4(), QRectF(1920, 0, 1920, 1080));
    tracker.addViewport(viewport1);
    tracker.addViewport(viewport2);
    viewport1->finishFrame();
    viewport2->finishFrame();
    auto bufferDamage = [&](const WSGViewport &v) {
        WPixmanRegion r =
            v.damageRegion().region.mappedOuter(sceneToBuffer(v, buf));
        r &= QRect(QPoint(), buf);
        return r;
    };
    auto commitOutputs = [&] {
    ...
    commitOutputs();
    QVERIFY(viewport1->isDirty());
    QVERIFY(viewport2->isDirty());
...
void WSGDamageGraphTest::viewportDirtyUsesVisibleSourceRegion()
{
    WSGViewport viewport;
    viewport.setRenderParameters(QMatrix4x4(), QRectF(50, 0, 50, 100),
                                 QRectF(-50, 0, 100, 100));
    viewport.finishFrame();

    viewport.addDamage(WDamageRegion(WPixmanRegion(QRect(10, 10, 10, 10)), false));
    QVERIFY(!viewport.isDirty());

    viewport.finishFrame();
    viewport.addDamage(WDamageRegion(WPixmanRegion(QRect(60, 10, 10, 10)), false));
    QVERIFY(viewport.isDirty());
}
...
void WSGDamageGraphTest::highDpiScaledOutputMapping()
{
    ...
    auto mapped = [&] {
        WPixmanRegion r =
            viewport->damageRegion().region.mappedOuter(sceneToBuffer(*viewport, buf, dpr));
        r &= QRect(QPoint(), buf);
        return r;
    };
...
void WSGDamageGraphTest::rotatedOutputMapping()
{
    ...
    auto mapped = [&] {
        WPixmanRegion r =
            viewport->damageRegion().region.mappedOuter(sceneToBuffer(*viewport, buf));
        r &= QRect(QPoint(), buf);
        return r;
    };
    WSGDamageTracker tracker(&root);
...
void WSGDamageGraphTest::viewportRegistrationLifecycle()
{
    WSGDamageNode root;
    auto *g = new WSGDamageGeometryNode;
    g->setBoundingRect(QRectF(10, 10, 20, 20));
    ...
    auto vp1 = std::make_shared<WSGViewport>();
    auto vp2 = std::make_shared<WSGViewport>();

    // 1. Initial registration
    tracker.addViewport(vp1);
    tracker.addViewport(vp2);
    // Initial commit establishes the baseline with g at (10, 10, 20, 20)
    tracker.commit();
    tracker.finishFrame();
    vp1->finishFrame();
    vp2->finishFrame();

    // 2. Duplicate registration is a no-op
    tracker.addViewport(vp1);

    // 3. One commit distributes damage to all registered viewports
    g->setBoundingRect(QRectF(20, 20, 30, 30));
    tracker.commit();
    QVERIFY(vp1->isDirty());
    QVERIFY(vp2->isDirty());
    COMPARE_REGION(vp1->damageRegion().region, QRegion(10, 10, 20, 20) + QRegion(20, 20, 30, 30));
    COMPARE_REGION(vp2->damageRegion().region, QRegion(10, 10, 20, 20) + QRegion(20, 20, 30, 30));

    // 5. Idempotent commit within same frame
    tracker.commit();
    COMPARE_REGION(vp1->damageRegion().region, QRegion(10, 10, 20, 20) + QRegion(20, 20, 30, 30));

    // 6. Idempotent finishFrame
    tracker.finishFrame();
    tracker.finishFrame();

    // 7. Viewport removal
    tracker.removeViewport(vp2.get());
    vp1->finishFrame();
    vp2->finishFrame();

    g->markContentDirty(QRect(20, 20, 5, 5));
    tracker.commit();
    tracker.finishFrame();
    QVERIFY(vp1->isDirty());
    QVERIFY(!vp2->isDirty());

    // 8. Expired weak_ptr cleanup
    {
        auto vp3 = std::make_shared<WSGViewport>();
        tracker.addViewport(vp3);
        // vp3 goes out of scope and is destroyed
    }
    // commit should prune the expired viewport without crash
    tracker.commit();
    tracker.finishFrame();
}
