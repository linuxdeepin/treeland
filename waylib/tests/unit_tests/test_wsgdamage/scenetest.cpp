...
static void applySceneDamage(WSGBatchRenderer::Renderer *batch,
                             const QRect &sceneOutputRect,
                             const WPixmanRegion &extraGpu = {})
{
    WPixmanRegion scene;
    ...
    if (tracker) {
        ...
        scene = viewport.damageRegion().region;
        viewport.finishFrame();
        if (!sceneOutputRect.isEmpty())
            scene &= sceneOutputRect;
    }
    WPixmanRegion gpu = scene;
    gpu += extraGpu;
    const auto coversOutput = [&](const WPixmanRegion &r) {
        return !sceneOutputRect.isEmpty() && (WPixmanRegion(sceneOutputRect) - r).isEmpty();
    };
    const WDamageRegion flushDamage =
        coversOutput(scene) ? WDamageRegion(true) : WDamageRegion(scene, false);
    const WDamageRegion renderDamage =
        coversOutput(gpu) ? WDamageRegion(true) : WDamageRegion(gpu, false);
    batch->setDamage(flushDamage, renderDamage, sceneOutputRect);
    if (tracker)
        tracker->finishFrame();
}
...
struct Snapshot
{
    ...
    QString dump() const
    {
        ...
        return WSGDamageLog::describe(region, full);
    }
    ...
    int area() const
    {
        ...
        for (int i = 0; i < count && boxes; ++i)
            total += (boxes[i].x2 - boxes[i].x1) * (boxes[i].y2 - boxes[i].y1);
        return total;
    }
};
...
private:
    ...
    Snapshot take()
    {
        ...
        auto *batch = renderer();
        if (batch) {
            if (batch->deviceRect().isEmpty())
                batch->setDeviceRect(QRect(0, 0, kSceneWidth, kSceneHeight));
            applySceneDamage(batch, QRect(0, 0, kSceneWidth, kSceneHeight));
            snap.valid = true;
            snap.full = batch->lastRenderRegion().isFull;
            snap.region = batch->lastRenderRegion().region;
        }
        m_rc->endFrame();
        return snap;
    }
    ...
    {
        QVERIFY2(snap.valid,
                 qPrintable(QStringLiteral("%1: %2").arg(QLatin1String(what), snap.dump())));
    }
    ...
    void expectCovers(const Snapshot &snap, const QRect &rect, const char *what)
    {
        ...
        if (probe.isEmpty())
            probe = rect;
        QVERIFY2((WPixmanRegion(probe) - snap.region).isEmpty(),
...
private Q_SLOTS:
    ...
    void viewportBufferDamage_onIdleTree();
    void rectangleColorChange_damagesOnlyTarget();
    void subtreeAggregation_damagesWholeBounds();
    void backgroundDamage_underTransparentContent();
    void backgroundDamage_underOpaqueContent();
    void backgroundDamage_offsetAndBounds();
};
...
void WSGDamageSceneTest::initTestCase()
{
    ...
    // The vulkan child needs an explicit instance attached before the render
    // control initializes; skip only when creation itself fails.
    if (qgetenv("QSG_RHI_BACKEND") == "vulkan") {
        m_vulkan = std::make_unique<QVulkanInstance>();
        if (!m_vulkan->create())
            QSKIP("QVulkanInstance::create() failed, skipping the Vulkan child");
    }
    ...
    if (m_vulkan)
        m_window->setVulkanInstance(m_vulkan.get());
    ...
    if (!m_rc->initialize()) {
        qWarning("QQuickRenderControl::initialize() failed (api=%d)",
                 int(m_window->rendererInterface() ? m_window->rendererInterface()->graphicsApi()
...
void WSGDamageSceneTest::transformItem_unionsOldAndNew()
{
    ...
    QVERIFY(target);
    const QRect oldRect = sceneRect(target);
    QVERIFY(QMetaObject::invokeMethod(m_root,
                                      "translateTarget",
                                      Qt::DirectConnection,
                                      Q_ARG(QVariant, 40),
                                      Q_ARG(QVariant, 30)));
...
void WSGDamageSceneTest::caretBlink_doesNotCoverButtons()
{
    ...
    expectCovers(snap, sceneRect(cursor), "caret");
    ...
    QVERIFY2(
        snap.area() <= 220 * 30 * 2,
        qPrintable(
            QStringLiteral("caret: area %1 too large (%2)").arg(snap.area()).arg(snap.dump())));
}
...
void WSGDamageSceneTest::idle_isEmptyOrTiny()
{
    ...
    QVERIFY2(!snap.full, qPrintable(QStringLiteral("idle: full damage %1").arg(snap.dump())));
...
void WSGDamageSceneTest::trackingDisabled_reportsFull()
{
    auto *batch = renderer();
    QVERIFY(batch);
    const auto was = batch->damageMode();
    batch->setDamageMode(WSGBatchRenderer::Renderer::DamageMode::Off);
    const QScopeGuard restore([batch, was] {
        batch->setDamageMode(was);
    });

    QVERIFY(batch->damageMode() == WSGBatchRenderer::Renderer::DamageMode::Off);
    auto *target = named("target");
    QVERIFY(target);
    target->setX(target->x() + 12);
    const Snapshot snap = take();
    expectValid(snap, "tracking off");
    QVERIFY2(
        snap.full,
        qPrintable(QStringLiteral("tracking off: expected full damage, got %1").arg(snap.dump())));
}

void WSGDamageSceneTest::viewportBufferDamage_onIdleTree()
{
    const Snapshot idle = take();
    expectValid(idle, "viewport damage settle");
    QVERIFY2(!idle.full, qPrintable(QStringLiteral("settle was full: %1").arg(idle.dump())));

    auto *batch = renderer();
    QVERIFY(batch);
    const QRect bufferRect(50, 50, 40, 40);
    applySceneDamage(batch, QRect(0, 0, kSceneWidth, kSceneHeight), WPixmanRegion(bufferRect));

    Snapshot snap;
    snap.valid = true;
    snap.full = batch->lastRenderRegion().isFull;
    snap.region = batch->lastRenderRegion().region;
    expectCovers(snap, bufferRect, "viewport buffer damage");
    QVERIFY2(!batch->lastFlushRegion().isFull,
             "swapchain buffer damage must not mark the ring full");
    QVERIFY2(batch->lastFlushRegion().region.isEmpty(),
             qPrintable(QStringLiteral("flush leaked buffer damage: %1")
                            .arg(WSGDamageLog::describe(batch->lastFlushRegion().region, false))));
}

void WSGDamageSceneTest::rectangleColorChange_damagesOnlyTarget()
{
    auto *target = named("target");
    QVERIFY(target);
    target->setProperty("color", QColor("#22aa66"));
    const Snapshot snap = take();

    expectCovers(snap, sceneRect(target), "color change");
    expectAvoids(snap, sceneRect(named("sentinel")), "color sentinel");
    expectAvoids(snap, sceneRect(named("btnA")), "color button");
}

void WSGDamageSceneTest::subtreeAggregation_damagesWholeBounds()
{
    auto *field = named("field");
    auto *cursor = named("cursor");
    auto *batch = renderer();
    QVERIFY(field);
    QVERIFY(cursor);
    ...
    QSGNode *subtreeRoot = QQuickItemPrivate::get(field)->itemNode();
    QVERIFY(subtreeRoot);
    QVERIFY(batch->setDamageSubtreeAggregation(subtreeRoot, true, field->boundingRect()));
    bool aggregationEnabled = true;
    const QScopeGuard restore([batch, subtreeRoot, &aggregationEnabled] {
        if (aggregationEnabled)
            batch->setDamageSubtreeAggregation(subtreeRoot, false);
    });
    take();

    cursor->setOpacity(0);
    const Snapshot aggregated = take();
    const QRect fieldRect = sceneRect(field);
    expectCovers(aggregated, fieldRect, "aggregate descendant content");
    expectAvoids(aggregated, sceneRect(named("sentinel")), "aggregate sentinel");
    expectAvoids(aggregated, sceneRect(named("btnA")), "aggregate button");
    expectBoundedBy(aggregated, fieldRect, "aggregate bounds");

    QVERIFY(batch->setDamageSubtreeAggregation(subtreeRoot, false));
    aggregationEnabled = false;
    take();

    cursor->setOpacity(1);
    const Snapshot exact = take();
    expectCovers(exact, sceneRect(cursor), "restored exact descendant");
    expectAvoids(exact, sceneRect(named("btnA")), "restored exact button");
    expectBoundedBy(exact, sceneRect(cursor), "restored exact bounds");
}

void WSGDamageSceneTest::backgroundDamage_underTransparentContent()
{
    auto *batch = renderer();
    ...
    auto *background = batch->backgroundDamageNode();
    QVERIFY(background);
    background->setBoundingRect(QRectF(0, 0, kSceneWidth, kSceneHeight));
    take();

    const QRect dirty(90, 90, 12, 10);
    background->markContentDirty(dirty);
    const Snapshot snap = take();
    expectNotFull(snap, "background under transparent item");
    QCOMPARE(snap.region.toQRegion(), QRegion(dirty));
    QCOMPARE(batch->lastFlushRegion().region.toQRegion(), QRegion(dirty));
    QVERIFY(take().region.isEmpty());
}

void WSGDamageSceneTest::backgroundDamage_underOpaqueContent()
{
    auto *batch = renderer();
    auto *wallpaper = named("wallpaper");
    auto *target = named("target");
    ...
    const QRect dirty(70, 90, 40, 10);
    background->markContentDirty(dirty);
    const Snapshot snap = take();
    const QRegion expected = QRegion(dirty) - QRegion(sceneRect(target));
    expectNotFull(snap, "background crosses opaque item");
    QCOMPARE(snap.region.toQRegion(), expected);
    QCOMPARE(batch->lastFlushRegion().region.toQRegion(), expected);

    background->markContentDirty(QRect(90, 90, 12, 10));
    const Snapshot covered = take();
    expectNotFull(covered, "background hidden by opaque item");
    QVERIFY(covered.region.isEmpty());
    QVERIFY(batch->lastFlushRegion().region.isEmpty());
}

void WSGDamageSceneTest::backgroundDamage_offsetAndBounds()
{
    auto *batch = renderer();
    auto *wallpaper = named("wallpaper");
    QVERIFY(batch);
    ...
    auto *background = batch->backgroundDamageNode();
    QVERIFY(background);
    const QRect bounds(300, 100, 80, 60);
    background->setBoundingRect(bounds);
    take();

    background->markContentDirty(QRect(10, 12, 8, 7));
    const Snapshot offset = take();
    expectNotFull(offset, "background content-local offset");
    QCOMPARE(offset.region.toQRegion(), QRegion(310, 112, 8, 7));

    background->markContentDirty(QRect(-8, -5, 100, 80));
    const Snapshot clipped = take();
    expectNotFull(clipped, "background clipped to input bounds");
    QCOMPARE(clipped.region.toQRegion(), QRegion(bounds));
    QCOMPARE(batch->lastFlushRegion().region.toQRegion(), QRegion(bounds));

    background->markContentDirty(QRect(90, 70, 10, 10));
    const Snapshot outside = take();
    expectNotFull(outside, "background outside input bounds");
    QVERIFY(outside.region.isEmpty());
}

QStringList damageSceneTestFunctionNames()
{
    QStringList names;
    const QMetaObject *meta = &WSGDamageSceneTest::staticMetaObject;
    for (int i = meta->methodOffset(); i < meta->methodCount(); ++i) {
        const QMetaMethod method = meta->method(i);
        if (method.methodType() == QMetaMethod::Slot)
            names << QString::fromLatin1(method.name());
    }
    return names;
}
