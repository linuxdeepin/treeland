                             ...
                             const WPixmanRegion &extraGpu = {})
{
    ...
    if (tracker) {
        auto viewport = std::make_shared<WSGViewport>();
...
#ifdef TEST_HAS_X11
    // Broken GLX (BadValue on GLXCreateNewContext) aborts via the default
    // X handler instead of failing QRhi. Ignore so initialize() can QSKIP.
...
void WSGDamageSceneTest::trackingDisabled_reportsFull()
{
    ...
    batch->setDamageMode(was);
    QVERIFY(batch->damageTracker());
    const Snapshot resumed = take();
    expectValid(resumed, "tracking resumed");
    QVERIFY2(resumed.full,
             qPrintable(QStringLiteral("tracking resumed: expected full damage, got %1")
                            .arg(resumed.dump())));
}
