...
private:
    ...
    void expectCovers(const Snapshot &snap, const QRect &rect, const char *what)
    {
        ...
        if (probe.isEmpty())
            probe = rect;
        QVERIFY2(QRegion(probe).subtracted(snap.region).isEmpty(),
...
void WSGDamageSceneTest::trackingDisabled_reportsFull()
{
    auto *batch = renderer();
    QVERIFY(batch);
    const bool was = batch->damageTrackingEnabled();
    batch->setDamageTrackingEnabled(false);
    const QScopeGuard restore([batch, was] {
        batch->setDamageTrackingEnabled(was);
    });

    QVERIFY(!batch->damageTrackingEnabled());
    auto *target = named("target");
    QVERIFY(target);
...
void WSGDamageSceneTest::viewportBufferDamage_onIdleTree()
{
    ...
    batch->setViewportDamage(QRegion(bufferRect));
    batch->prepareDamageFrame();
            batch->commitPendingDamage();
    batch->finishDamageFrame();
    ...
    QVERIFY2(!batch->sceneFlushRegionIsFull(),
             "swapchain buffer damage must not mark the ring full");
    QVERIFY2(batch->sceneFlushRegion().isEmpty(),
             qPrintable(QStringLiteral("sceneFlush leaked buffer damage: %1")
                            .arg(WSGDamageLog::describe(batch->sceneFlushRegion(), false))));
}
