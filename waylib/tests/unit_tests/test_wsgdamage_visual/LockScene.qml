...
Item {
    ...
    // Deterministic damage tests set testMode during setup so the 150ms
    // chip expand Behaviors below never run: geometry settles in the frame
    // it changes instead of racing frame capture.
    ...
    Item {
        ...
        Button {
            ...
            Behavior on width { enabled: !root.testMode; NumberAnimation { duration: 150 } }
            Behavior on height { enabled: !root.testMode; NumberAnimation { duration: 150 } }
