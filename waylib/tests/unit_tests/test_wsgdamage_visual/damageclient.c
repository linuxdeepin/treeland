...
{
    pthread_mutex_lock(&c->err_mutex);
    snprintf(c->error, sizeof(c->error), "%s", msg);
    pthread_mutex_unlock(&c->err_mutex);
}
...
static int create_shm_buffer(struct DamageClient *c, struct ShmBuffer *out)
{
    if (c->width <= 0 || c->height <= 0)
        return -1;
    const size_t size = (size_t)c->width * 4u * (size_t)c->height;
    if (size == 0 || size > (size_t)INT32_MAX)
        return -1;
    const int stride = (int)((size_t)c->width * 4u);
...
static void destroy_pixel_storage(struct DamageClient *c)
{
...
static int create_pixel_storage(struct DamageClient *c)
{
    if (c->width <= 0 || c->height <= 0)
        return -1;
    for (int i = 0; i < kBufferCount; ++i) {
        if (create_shm_buffer(c, &c->buffers[i]) < 0) {
            destroy_pixel_storage(c);
            return -1;
        }
    }
    c->shadow_size = (size_t)c->width * (size_t)c->height * sizeof(uint32_t);
    c->shadow = malloc(c->shadow_size);
    if (!c->shadow) {
        destroy_pixel_storage(c);
        return -1;
    }
    for (size_t i = 0; i < c->shadow_size / sizeof(uint32_t); ++i)
        c->shadow[i] = (uint32_t)kBasePixel;
    return 0;
}

/* A configure only stages the new size; the old buffers stay valid until the
 * next commit recreates every pixel store up front, so in-flight pixels and
 * shadow indexing never disagree about the dimensions. */
static int apply_pending_resize(struct DamageClient *c)
{
    const int width = c->pending_width > 0 ? c->pending_width : c->width;
    const int height = c->pending_height > 0 ? c->pending_height : c->height;
    c->pending_width = 0;
    c->pending_height = 0;
    ...
    c->width = width;
    c->height = height;
    if (create_pixel_storage(c) < 0) {
        set_error(c, "pixel storage recreate failed");
        return -1;
    }
    return 0;
}
...
static struct ShmBuffer *acquire_buffer(struct DamageClient *c)
{
    struct ShmBuffer *buf = pick_buffer(c);
    if (buf || !c->display || !atomic_load(&c->running))
        return buf;

    /* Deliver already-queued releases without blocking. */
    wl_display_dispatch_pending(c->display);
    buf = pick_buffer(c);
    if (buf)
        return buf;

    /* Never block here: the caller sits in damage_client_commit_damage waiting
     * for our ACK while the in-process server needs that same thread to make
     * progress, so waiting for a release would deadlock both sides. Drain only
     * what already arrived and let the caller retry after pumping instead. */
    ...
    return pick_buffer(c);
}

static void fill_rect_pixels(uint32_t *pixels, int width, int x, int y, int w, int h, uint32_t pixel)
{
    for (int row = y; row < y + h; ++row) {
        uint32_t *line = pixels + row * width + x;
                                   ...
                                   int32_t width, int32_t height, struct wl_array *states)
{
    ...
    struct DamageClient *c = data;
    if (width > 0)
        c->pending_width = width;
    if (height > 0)
        c->pending_height = height;
}
...
{
    if (apply_pending_resize(c) < 0)
        return -1;
    if (x < 0 || y < 0 || w <= 0 || h <= 0)
        return -1;
    if (x > c->width || y > c->height)
        return -1;
    if (w > c->width - x || h > c->height - y)
        return -1;

    struct ShmBuffer *buf = acquire_buffer(c);
    if (!buf || !c->shadow)
        return -1;
    fill_rect_pixels(c->shadow, c->width, x, y, w, h, (uint32_t)kDamagePixel);
    memcpy(buf->pixels, c->shadow, c->shadow_size);
    buf->busy = 1;
...
struct DamageClient *damage_client_create(int fd)
{
    ...
    if (pthread_mutex_init(&c->err_mutex, NULL) != 0) {
        close(fd);
        free(c);
        return NULL;
    }
    ...
    if (pipe(cmd) < 0 || pipe(ack) < 0) {
        ...
        if (ack[1] >= 0)
            close(ack[1]);
        close(fd);
        pthread_mutex_destroy(&c->err_mutex);
        free(c);
        return NULL;
    }
    c->cmd_rd = cmd[0];
...
void damage_client_run(struct DamageClient *c)
{
    ...
    if (c->pending_width > 0)
        c->width = c->pending_width;
    if (c->pending_height > 0)
...
{
    static _Thread_local char snapshot[sizeof(((struct DamageClient *)0)->error)];
    if (!client)
        return "null client";
    pthread_mutex_lock((pthread_mutex_t *)&client->err_mutex);
    snprintf(snapshot, sizeof(snapshot), "%s", client->error);
    pthread_mutex_unlock((pthread_mutex_t *)&client->err_mutex);
...
void damage_client_destroy(struct DamageClient *client)
{
    ...
    pthread_mutex_destroy(&client->err_mutex);
    free(client);
}
