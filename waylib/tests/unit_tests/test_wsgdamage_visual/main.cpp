...
private:
    ...
    FrameSnap captureOutput(bool grabScene, bool forceViewportRender = true)
    {
        ...
        const QMetaObject::Connection conn = connect(
            m_window, &QQuickWindow::afterRendering, this, [&] {
                ...
                if (auto *batch = dynamic_cast<WSGBatchRenderer::Renderer*>(br->renderer())) {
        ...
        } else if (auto *output = m_viewport->output()) {
            wlr_output_send_frame(output->handle());
        }
        disconnect(conn);
        ...
        auto *br = WOutputViewportPrivate::get(m_viewport)->bufferRenderer;
        if (br && !gotBatchFlush) {
            snap.full = br->lastRenderRegion().isFull;
            snap.region = br->lastRenderRegion().region;
        }
    ...
    bool forceFullFlush()
    {
        auto *privateViewport = WOutputViewportPrivate::get(m_viewport);
        if (!privateViewport->bufferRenderer)
            return false;
        privateViewport->viewport->markFull();
        return true;
    }
    ...
    bool usesTrackedPartialDamage() const
    {
        ...
        if (WRenderHelper::getGraphicsApi() == QSGRendererInterface::Software)
            return false;
        return WSGBatchRenderer::Renderer::defaultDamageMode() != WSGBatchRenderer::Renderer::DamageMode::Off;
    }
    ...
    bool expectRegionCovers(const FrameSnap &snap, const QRect &rect, const char *what)
    {
        ...
        if (probe.isEmpty())
            probe = rect;
        return QTest::qVerify((WPixmanRegion(probe) - snap.region).isEmpty(),
                              "region covers",
    ...
    bool expectRegionNotFullscreen(const FrameSnap &snap, const char *what)
    {
        ...
        // Quarter of the committed output buffer, not a hard-coded 800x480.
        const int kMaxArea = snap.buffer.width() * snap.buffer.height() / 4;
...
void WSGDamageDebugVisualTest::frosted_behindMoveMatchesRerender()
{
    ...
    QVERIFY(expectRegionCovers(partial, itemOutputRect(behind), "frost blitter behind"));
    QVERIFY(expectRegionAvoids(partial, itemOutputRect(named("sentinel")), "frost sentinel"));
...
void WSGDamageDebugVisualTest::highlight_softwareInlineCursorMoveRegion()
{
    ...
    QVERIFY(expectRegionCovers(snap, oldRect, "inline software cursor old"));
    QVERIFY(expectRegionCovers(snap, now, "inline software cursor new"));
...
void WSGDamageDebugVisualTest::client_commitDamageRegion()
{
    ...
    // Reuse captureOutput: Vulkan only submits the pass in endFrame, after
    // afterRendering, so reading lastRenderRegion off a raw render() misses it.
    const FrameSnap snap = captureOutput(false);
    QVERIFY2(snap.valid, "output dump failed after the client damage commit");
...
void WSGDamageDebugVisualTest::lockscreen_idlePasswordBlur_stableAcrossFrames()
{
    ...
    if (qEnvironmentVariableIsSet("WSG_DAMAGE_SAVE_SCREENS")) {
...
void WSGDamageDebugVisualTest::lockscreen_sessionChipBlur_afterHideSourceMatchesRerender()
{
    ...
    const FrameSnap partial = captureOrSkip(false);
    // Bottom-right corner derived from the committed buffer, like the idle test above.
    const QRect corner(partial.buffer.width() - 220, partial.buffer.height() - 90, 210, 80);
    if (qEnvironmentVariableIsSet("WSG_DAMAGE_SAVE_SCREENS")) {
...
int main(int argc, char *argv[])
{
    if (!qEnvironmentVariableIsSet("WSG_DAMAGE_TEST_CHILD")
        ...
        // Default to gles2 only: the full gles2/vulkan/pixman matrix triples
        // CTest wall-clock time past the timeout. Opt back into the full
        // matrix with WSG_DAMAGE_TEST_ALL_RENDERERS=1.
        QStringList rendererList{ QStringLiteral("gles2") };
        if (qEnvironmentVariableIsSet("WSG_DAMAGE_TEST_ALL_RENDERERS"))
            rendererList << QStringLiteral("vulkan") << QStringLiteral("pixman");
        for (const QString &renderer : rendererList) {
            QProcessEnvironment env = QProcessEnvironment::systemEnvironment();
            env.insert(QStringLiteral("WLR_RENDERER"), renderer);
            env.insert(QStringLiteral("WSG_DAMAGE_TEST_CHILD"), QStringLiteral("1"));
            if (renderer == QLatin1String("gles2")) {
